...
var rootCmd = &cobra.Command{
	Use:           "operica",
	Short:         "Operica CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Operica from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("operica {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Operica server URL (env: OPERICA_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: OPERICA_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: OPERICA_DEBUG)")
