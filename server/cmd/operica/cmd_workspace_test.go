...
func TestRunWorkspaceCreatePostsWorkspaceAndDoesNotSwitchDefault(t *testing.T) {
	...
	t.Setenv("OPERICA_WORKSPACE_ID", "existing-workspace")
...
func TestRunWorkspaceSwitch(t *testing.T) {
	...
	// Isolate HOME so the test never touches the developer's ~/.operica.
	t.Setenv("HOME", t.TempDir())
	t.Setenv("OPERICA_SERVER_URL", srv.URL)
	t.Setenv("OPERICA_TOKEN", "test-token")
	...
	t.Run("isolates by profile", func(t *testing.T) {
		...
		wantSuffix := filepath.Join(".operica", "profiles", "staging", "config.json")
...
func TestRunWorkspaceSwitchFailsClosedInTaskContext(t *testing.T) {
	...
	t.Setenv("OPERICA_WORKSPACE_ID", "task-workspace")
