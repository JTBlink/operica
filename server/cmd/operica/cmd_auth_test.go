...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers OPERICA_APP_URL", func(t *testing.T) {
		t.Setenv("OPERICA_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://operica.ai/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `operica login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}

// TestLoginTokenHelpOutputRendersCleanly renders loginCmd's flag help through
// the same pflag path `operica login -h` uses (FlagUsagesWrapped) and locks the
...
func TestRunAuthStatusTaskContextRequiresTaskToken(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("OPERICA_TOKEN", tc.token)
			requestCount = 0
			stderr := captureStderr(t)
...
func TestHumanAuthCommandsFailClosedInTaskContext(t *testing.T) {
	...
	t.Setenv("OPERICA_AGENT_ID", "agent-test")
	t.Setenv("OPERICA_TASK_ID", "task-test")
	t.Setenv("OPERICA_TOKEN", "mat_task_sentinel")
	t.Setenv("OPERICA_SERVER_URL", "https://task.invalid")
	t.Setenv("OPERICA_TASK_CONFIG_ROOT", filepath.Join(t.TempDir(), "task-operica"))
...
// `operica login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate Operica Cloud Node PATs (`mcn_`) at
