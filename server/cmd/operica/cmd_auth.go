...
// loginTokenPrefixes are the token prefixes `operica login --token` accepts.
// The CLI used to hardcode `mul_` only, which made it impossible to log in
// with a Operica Cloud Node PAT (`mcn_`) even though the server happily
...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate operica with Operica",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("OPERICA_TOKEN")); v != "" {
		return v
	}
	...
	// inDaemonManagedExecutionContext already covers the OPERICA_DAEMON_PORT
	// signal for subprocesses that lost OPERICA_AGENT_ID / OPERICA_TASK_ID.
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"OPERICA_APP_URL", "FRONTEND_ORIGIN"} {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'operica setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthLoginBrowser(cmd *cobra.Command) error {
	...
	if err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server could not issue an access token for the CLI. Run `operica login` again.", err)
	}
	...
	if err := patClient.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `operica login` again.", err)
	}
...
func runAuthLoginToken(cmd *cobra.Command, providedToken string) error {
	...
	if err := client.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Could not sign in with that token — make sure it is valid and not expired, then run `operica login --token <token>` again.", err)
	}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if taskContext && !strings.HasPrefix(token, "mat_") {
		return fmt.Errorf("agent execution context requires OPERICA_TOKEN to be a task-scoped mat_ token")
	}
	serverURL := resolveServerURL(cmd)

	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'operica login' to authenticate.")
		return nil
	}
...
<title>Operica — Authenticated</title>
<style>
