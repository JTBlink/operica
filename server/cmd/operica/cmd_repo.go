...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("OPERICA_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("OPERICA_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("OPERICA_WORKSPACE_ID")
	agentName := os.Getenv("OPERICA_AGENT_NAME")
	taskID := os.Getenv("OPERICA_TASK_ID")
	...
	reqBody := map[string]string{
		...
		"task_id":       taskID,
		"checkout_mode": strings.TrimSpace(os.Getenv("OPERICA_REPO_CHECKOUT_MODE")),
	}
