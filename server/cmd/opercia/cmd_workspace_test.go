...
func TestRunWorkspaceCreatePostsWorkspaceAndDoesNotSwitchDefault(t *testing.T) {
	...
	t.Setenv("OPERCIA_WORKSPACE_ID", "existing-workspace")
...
func TestRunWorkspaceSwitch(t *testing.T) {
	...
	// Isolate HOME so the test never touches the developer's ~/.opercia.
	t.Setenv("HOME", t.TempDir())
	t.Setenv("OPERCIA_SERVER_URL", srv.URL)
	t.Setenv("OPERCIA_TOKEN", "test-token")
	...
	t.Run("isolates by profile", func(t *testing.T) {
		...
		wantSuffix := filepath.Join(".opercia", "profiles", "staging", "config.json")
...
func TestRunWorkspaceSwitchFailsClosedInTaskContext(t *testing.T) {
	...
	t.Setenv("OPERCIA_WORKSPACE_ID", "task-workspace")
