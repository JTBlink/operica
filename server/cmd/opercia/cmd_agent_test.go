...
// impersonation escape: a sandbox fault strips every OPERCIA_* env var from
// an agent subprocess, which then runs `opercia` from the *parent* directory
...
func TestNewAPIClient_WorkdirParentEscapeFailsClosed(t *testing.T) {
	...
	// Daemon-owned tree: {root}/.opercia marker + {root}/{ws}/{task}/workdir
	...
	t.Setenv("OPERCIA_AGENT_ID", "")
	t.Setenv("OPERCIA_TASK_ID", "")
	t.Setenv("OPERCIA_DAEMON_PORT", "")
	t.Setenv("OPERCIA_TOKEN", "")
	t.Setenv("OPERCIA_SERVER_URL", "http://127.0.0.1:8080")
...
// agent task (OPERCIA_AGENT_ID / OPERCIA_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.opercia/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("OPERCIA_AGENT_ID", "agent-123")
		t.Setenv("OPERCIA_TASK_ID", "task-456")
		t.Setenv("OPERCIA_DAEMON_PORT", "")
		t.Setenv("OPERCIA_WORKSPACE_ID", "env-ws")
	...
	t.Run("daemon port marker also skips config", func(t *testing.T) {
		t.Setenv("OPERCIA_AGENT_ID", "")
		t.Setenv("OPERCIA_TASK_ID", "")
		t.Setenv("OPERCIA_DAEMON_PORT", "27182")
		t.Setenv("OPERCIA_WORKSPACE_ID", "")
...
func TestResolveToken_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in agent context without OPERCIA_TOKEN", got)
		}
	})
	...
	t.Run("daemon port marker without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in daemon-managed context without OPERCIA_TOKEN", got)
		}
	})
	...
	t.Run("agent context uses explicit task token env", func(t *testing.T) {
		...
		t.Setenv("OPERCIA_TASK_ID", "task-456")
		t.Setenv("OPERCIA_DAEMON_PORT", "")
		t.Setenv("OPERCIA_TOKEN", "mat_task_token")

		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want OPERCIA_TOKEN", got)
		}
	})
	...
	t.Run("daemon port set without agent context avoids config fallback", func(t *testing.T) {
		...
		t.Setenv("OPERCIA_TASK_ID", "")
		t.Setenv("OPERCIA_TOKEN", "")
		t.Setenv("OPERCIA_DAEMON_PORT", "19514")
	...
	t.Run("daemon port set with explicit task token uses task token", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want OPERCIA_TOKEN (task token wins over daemon signal)", got)
		}
	})

	// OPERCIA_SERVER_URL is a user-facing env var that may be set in a
	// normal shell. It is NOT a daemon identity signal — only
	// OPERCIA_DAEMON_PORT is. The config fallback must still work when
	// SERVER_URL is set but no daemon signal is present.
	t.Run("OPERCIA_SERVER_URL alone does not block config fallback", func(t *testing.T) {
		t.Setenv("OPERCIA_AGENT_ID", "")
		t.Setenv("OPERCIA_TASK_ID", "")
		t.Setenv("OPERCIA_TOKEN", "")
		t.Setenv("OPERCIA_DAEMON_PORT", "")
		t.Setenv("OPERCIA_SERVER_URL", "https://api.opercia.ai")
...
func TestNewAPIClient_AgentContextRequiresTaskToken(t *testing.T) {
	t.Setenv("OPERCIA_SERVER_URL", "http://127.0.0.1:8080")
	t.Setenv("OPERCIA_WORKSPACE_ID", "workspace-123")
	t.Setenv("OPERCIA_AGENT_ID", "agent-123")
	t.Setenv("OPERCIA_TASK_ID", "task-456")
	...
	t.Run("member token fails closed", func(t *testing.T) {
		t.Setenv("OPERCIA_TOKEN", "mul_member_token")
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("OPERCIA_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("OPERCIA_WORKSPACE_ID", "test-ws")
	t.Setenv("OPERCIA_TOKEN", "test-token")
	t.Setenv("OPERCIA_AGENT_ID", "")
	t.Setenv("OPERCIA_TASK_ID", "")
	...
	msg := err.Error()
	if !strings.Contains(msg, "opercia agent env set") {
		t.Fatalf("no-fields error must direct users to `opercia agent env set`; got: %q", msg)
	}
}
...
func TestAgentUpdateDoesNotExposeCustomEnvFlags(t *testing.T) {
	for _, flag := range []string{"custom-env", "custom-env-stdin", "custom-env-file"} {
		if agentUpdateCmd.Flag(flag) != nil {
			t.Errorf("agent update must NOT expose --%s after MUL-2600; use `opercia agent env set` instead", flag)
		}
	}
}
...
func TestAgentMaxConcurrentTasksFlagValidation(t *testing.T) {
	...
	t.Setenv("HOME", t.TempDir())
	t.Setenv("OPERCIA_SERVER_URL", srv.URL)
	t.Setenv("OPERCIA_WORKSPACE_ID", "ws-1")
	t.Setenv("OPERCIA_TOKEN", "test-token")
	t.Setenv("OPERCIA_AGENT_ID", "")
	t.Setenv("OPERCIA_TASK_ID", "")
