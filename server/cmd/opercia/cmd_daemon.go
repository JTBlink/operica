...
import (
	...
	logger_pkg "github.com/JTBlink/operica/server/internal/logger"
...
var daemonDiskUsageCmd = &cobra.Command{
	...
	Long: "Walks the daemon's workspaces root and reports per-task or per-workspace disk usage.\n" +
		...
		"root — the default root plus each ~/.opercia/profiles/* root, including the Desktop app's dedicated\n" +
		...
		"overridable via OPERCIA_GC_ARTIFACT_PATTERNS) so the report stays in sync with what the GC reclaims.\n" +
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: OPERCIA_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: OPERCIA_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: OPERCIA_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: OPERCIA_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: OPERCIA_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: OPERCIA_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: OPERCIA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: OPERCIA_CODEX_HANDSHAKE_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: OPERCIA_DAEMON_MAX_CONCURRENT_TASKS)")
	f.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: OPERCIA_DAEMON_AUTO_UPDATE=false)")
	f.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: OPERCIA_DAEMON_AUTO_UPDATE_INTERVAL)")
	f.Bool("no-auto-reload", false, "Disable restarting when the opercia binary on disk changes version (env: OPERCIA_DAEMON_AUTO_RELOAD=false)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: OPERCIA_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: OPERCIA_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: OPERCIA_AGENT_RUNTIME_NAME)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: OPERCIA_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: OPERCIA_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: OPERCIA_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: OPERCIA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: OPERCIA_CODEX_HANDSHAKE_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: OPERCIA_DAEMON_MAX_CONCURRENT_TASKS)")
	rf.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: OPERCIA_DAEMON_AUTO_UPDATE=false)")
	rf.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: OPERCIA_DAEMON_AUTO_UPDATE_INTERVAL)")
	rf.Bool("no-auto-reload", false, "Disable restarting when the opercia binary on disk changes version (env: OPERCIA_DAEMON_AUTO_RELOAD=false)")
	...
	df.Bool("all-profiles", false, "Scan every workspace root (default root + all ~/.opercia/profiles/* roots, incl. the Desktop app's) and report a combined total")
...
// Empty profile → ~/.opercia/, named profile → ~/.opercia/profiles/<name>/.
...
func newDaemonLogRotator(logPath string) *lumberjack.Logger {
	return &lumberjack.Logger{
		Filename:   logPath,
		MaxSize:    envPositiveIntOrDefault("OPERCIA_DAEMON_LOG_MAX_SIZE_MB", defaultDaemonLogMaxSizeMB),
		MaxBackups: envPositiveIntOrDefault("OPERCIA_DAEMON_LOG_MAX_BACKUPS", defaultDaemonLogMaxBackups),
		MaxAge:     envPositiveIntOrDefault("OPERCIA_DAEMON_LOG_MAX_AGE_DAYS", defaultDaemonLogMaxAgeDays),
		Compress:   true,
	}
}
...
// requireDaemonAuth fails fast when the user never ran `opercia login`. The
...
// authenticates via the stored config token, never OPERCIA_TOKEN.
func requireDaemonAuth(profile string) error {
	...
	if cfg.Token == "" {
		loginHint := "opercia login"
		if profile != "" {
			loginHint = fmt.Sprintf("opercia login --profile %s", profile)
		}
...
// OPERCIA_SERVER_URL env, then the stored CLI config for the profile.
func resolveDaemonServerURL(cmd *cobra.Command, profile string) string {
	serverURL := cli.FlagOrEnv(cmd, "server-url", "OPERCIA_SERVER_URL", "")
	if serverURL == "" {
...
func daemonStartupFailureError(logs daemonStartupLogs, waitErr error, profile, serverURL string) error {
	...
	case strings.Contains(joined, "connection refused") ||
		...
		target := "the Opercia server"
		if serverURL != "" {
			target += " at " + serverURL
		}
		return fmt.Errorf("daemon failed to start: cannot reach %s.\nMake sure the server is running and reachable, then rerun 'opercia daemon start'.\nFull log: %s", target, logs.logPath)
	}
...
func buildDaemonStartArgs(cmd *cobra.Command) []string {
	...
	// (= no cap), so it can override an environment OPERCIA_AGENT_TIMEOUT.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	//
	//   --flag  >  OPERCIA_… env  >  config.json  >  built-in default
	//
	...
	pollOverride, err := resolveDaemonDurationOverride(pollFlag, "OPERCIA_DAEMON_POLL_INTERVAL", fileCfg.PollInterval)
	if err != nil {
		return err
	}
	if pollOverride > 0 {
	...
	heartbeatOverride, err := resolveDaemonDurationOverride(heartbeatFlag, "OPERCIA_DAEMON_HEARTBEAT_INTERVAL", fileCfg.HeartbeatInterval)
	if err != nil {
		return err
	}
	...
	agentTimeoutOverride, err := resolveDaemonAgentTimeoutOverride(cmd, "OPERCIA_AGENT_TIMEOUT", fileCfg.AgentTimeout)
	if err != nil {
		return err
	}
	...
	semanticOverride, err := resolveDaemonDurationOverride(semanticFlag, "OPERCIA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", fileCfg.CodexSemanticInactivityTimeout)
	if err != nil {
		return err
	}
	...
	handshakeOverride, err := resolveDaemonDurationOverride(handshakeFlag, "OPERCIA_CODEX_HANDSHAKE_TIMEOUT", fileCfg.CodexHandshakeTimeout)
	if err != nil {
		return err
	}
	...
	if n := resolveDaemonIntOverride(maxFlag, "OPERCIA_DAEMON_MAX_CONCURRENT_TASKS", fileCfg.MaxConcurrentTasks); n > 0 {
		overrides.MaxConcurrentTasks = n
	}
	// --no-auto-update / OPERCIA_DAEMON_AUTO_UPDATE=false / config.json
	...
	if resolveDaemonDisableSignal(noAutoUpdateFlag, "OPERCIA_DAEMON_AUTO_UPDATE", fileCfg.DisableAutoUpdate) {
	...
	if resolveDaemonDisableSignal(noAutoReloadFlag, "OPERCIA_DAEMON_AUTO_RELOAD", fileCfg.DisableAutoReload) {
		overrides.DisableAutoReload = true
	}
	autoUpdateFlag, _ := cmd.Flags().GetDuration("auto-update-interval")
	autoUpdateOverride, err := resolveDaemonDurationOverride(autoUpdateFlag, "OPERCIA_DAEMON_AUTO_UPDATE_INTERVAL", fileCfg.AutoUpdateCheckInterval)
	if err != nil {
		return err
	}
	...
	cfg.LaunchedBy = os.Getenv("OPERCIA_LAUNCHED_BY")
...
// `opercia auth status`) and refuse to touch the running daemon on failure.
func requireDaemonRestartPreflight(cmd *cobra.Command, profile string) error {
	...
	if err := cli.NewAPIClient(baseURL, "", cfg.Token).GetJSON(ctx, "/api/me", nil); err != nil {
		...
		return fmt.Errorf("refusing to restart: cannot reach the Opercia server at %s (%w); the running daemon was left untouched.\nMake sure the server is running and reachable, then rerun 'opercia daemon restart'", baseURL, err)
	}
	return nil
}
...
// daemon-injected OPERCIA_DAEMON_PORT and nothing else: healthPortForProfile
...
func daemonStatusHealthPort(cmd *cobra.Command) (int, error) {
	...
	raw := strings.TrimSpace(os.Getenv("OPERCIA_DAEMON_PORT"))
	if raw == "" {
		return 0, fmt.Errorf("daemon status inside a daemon-managed task requires the daemon-injected OPERCIA_DAEMON_PORT")
	}
	port, err := strconv.Atoi(raw)
	if err != nil || port <= 0 {
		return 0, fmt.Errorf("invalid OPERCIA_DAEMON_PORT %q inside a daemon-managed task", raw)
	}
	return port, nil
}
...
//  2. OPERCIA_AGENT_TIMEOUT is set -> return nil so LoadConfig reads the
...
//  2. OPERCIA_DAEMON_AUTO_UPDATE explicitly set to a falsy value ->
...
// boundary: --all-profiles enumerates ~/.opercia/profiles and discloses the
...
// each ~/.opercia/profiles/* root) and renders a per-root breakdown with a
...
// orientation even when empty), then each ~/.opercia/profiles/* root that
// exists on disk, sorted by profile name. Roots that resolve to the same path
// (e.g. when OPERCIA_WORKSPACES_ROOT pins every profile to one directory) are
...
func printRepoCacheLine(w io.Writer, report daemon.DiskUsageReport) {
	...
	fmt.Fprintf(w, "Repo cache (.repos): %s across %d repo(s), not included above. Evicted once a repo is unused for OPERCIA_GC_REPO_TTL and no longer attached to a workspace.\n",
...
func printDiskUsageOtherRootsHint(w io.Writer, report daemon.DiskUsageReport, profile, rootOverride string, taskContext bool) {
	...
	// The suggestions are built by listing ~/.opercia/profiles, so printing
	...
	fmt.Fprintln(w, "Run 'opercia daemon disk-usage --all-profiles' for a combined total across all roots.")
}
...
func diskUsageProfileSuggestions(currentProfile, currentRoot string) []diskUsageProfileSuggestion {
		...
		if root, err := daemon.ResolveWorkspacesRoot("", ""); err == nil && !samePath(root, currentRoot) {
			if taskCount := countDiskUsageTaskDirs(root); taskCount > 0 {
				out = append(out, diskUsageProfileSuggestion{
					Profile:   "",
					Command:   "opercia daemon disk-usage",
					Root:      root,
					TaskCount: taskCount,
				})
			}
	...
	for _, entry := range entries {
		...
		out = append(out, diskUsageProfileSuggestion{
			Profile:   profile,
			Command:   "opercia --profile " + shellQuoteArg(profile) + " daemon disk-usage",
			Root:      root,
			TaskCount: taskCount,
		})
	}
...
func profilesRootDir() (string, error) {
	...
	return filepath.Join(home, ".opercia", "profiles"), nil
}
