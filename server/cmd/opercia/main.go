...
var rootCmd = &cobra.Command{
	Use:           "opercia",
	Short:         "Opercia CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Opercia from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("opercia {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Opercia server URL (env: OPERCIA_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: OPERCIA_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: OPERCIA_DEBUG)")
