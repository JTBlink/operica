...
// loginTokenPrefixes are the token prefixes `opercia login --token` accepts.
// The CLI used to hardcode `mul_` only, which made it impossible to log in
// with a Opercia Cloud Node PAT (`mcn_`) even though the server happily
...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate opercia with Opercia",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("OPERCIA_TOKEN")); v != "" {
		return v
	}
	...
	// inDaemonManagedExecutionContext already covers the OPERCIA_DAEMON_PORT
	// signal for subprocesses that lost OPERCIA_AGENT_ID / OPERCIA_TASK_ID.
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"OPERCIA_APP_URL", "FRONTEND_ORIGIN"} {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'opercia setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthLoginBrowser(cmd *cobra.Command) error {
	...
	if err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server could not issue an access token for the CLI. Run `opercia login` again.", err)
	}
	...
	if err := patClient.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `opercia login` again.", err)
	}
...
func runAuthLoginToken(cmd *cobra.Command, providedToken string) error {
	...
	if err := client.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Could not sign in with that token — make sure it is valid and not expired, then run `opercia login --token <token>` again.", err)
	}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if taskContext && !strings.HasPrefix(token, "mat_") {
		return fmt.Errorf("agent execution context requires OPERCIA_TOKEN to be a task-scoped mat_ token")
	}
	serverURL := resolveServerURL(cmd)

	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'opercia login' to authenticate.")
		return nil
	}
...
<title>Opercia — Authenticated</title>
<style>
