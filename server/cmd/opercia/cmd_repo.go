...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("OPERCIA_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("OPERCIA_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("OPERCIA_WORKSPACE_ID")
	agentName := os.Getenv("OPERCIA_AGENT_NAME")
	taskID := os.Getenv("OPERCIA_TASK_ID")
	...
	reqBody := map[string]string{
		...
		"task_id":       taskID,
		"checkout_mode": strings.TrimSpace(os.Getenv("OPERCIA_REPO_CHECKOUT_MODE")),
	}
