...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers OPERCIA_APP_URL", func(t *testing.T) {
		t.Setenv("OPERCIA_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://opercia.ai/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `opercia login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}

// TestLoginTokenHelpOutputRendersCleanly renders loginCmd's flag help through
// the same pflag path `opercia login -h` uses (FlagUsagesWrapped) and locks the
...
func TestRunAuthStatusTaskContextRequiresTaskToken(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("OPERCIA_TOKEN", tc.token)
			requestCount = 0
			stderr := captureStderr(t)
...
func TestHumanAuthCommandsFailClosedInTaskContext(t *testing.T) {
	...
	t.Setenv("OPERCIA_AGENT_ID", "agent-test")
	t.Setenv("OPERCIA_TASK_ID", "task-test")
	t.Setenv("OPERCIA_TOKEN", "mat_task_sentinel")
	t.Setenv("OPERCIA_SERVER_URL", "https://task.invalid")
	t.Setenv("OPERCIA_TASK_CONFIG_ROOT", filepath.Join(t.TempDir(), "task-opercia"))
...
// `opercia login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate Opercia Cloud Node PATs (`mcn_`) at
