...
func withAgentContext(t *testing.T) {
	t.Helper()
	t.Setenv("OPERCIA_TASK_ID", "task-1")
}
...
func TestGuardLocalPathLinksOnlyFiresInAgentContext(t *testing.T) {
	...
	t.Run("human PAT context is never linted", func(t *testing.T) {
		// No OPERCIA_AGENT_ID / OPERCIA_TASK_ID: a person running the CLI. Their
...
func TestGuardLocalPathLinksHintIsPerCommand(t *testing.T) {
	...
	err := guardLocalPathLinks(
		"[screenshot]("+shot+")",
		"issue description",
		"`opercia issue update` cannot carry files — deliver the file with `opercia issue comment add <issue-id> --attachment <path>` instead, and drop the link.",
	)
	if err == nil {
		t.Fatal("expected a hard failure")
	}
	msg := err.Error()
	if !strings.Contains(msg, "`opercia issue update` cannot carry files") {
		t.Errorf("update hint missing, got: %v", msg)
	}
	if !strings.Contains(msg, "opercia issue comment add <issue-id> --attachment <path>") {
		t.Errorf("update hint must redirect to comment add, got: %v", msg)
	}
	if strings.Contains(msg, "opercia issue update --attachment") {
