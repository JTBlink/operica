...
func (b *kiroBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// (goal_complete or `opercia issue comment add`). We record the status
	...
	c := &hermesClient{
		...
		onMessage: func(msg Message) {
			...
			if msg.Type == MessageToolUse {
				...
				// Recognize `opercia issue comment add` by its command
...
// `opercia issue comment add` invocation. It keys purely off the command
...
func isKiroIssueCommentAddCommand(command string) bool {
	...
	// `sh -c "opercia issue comment add ..."`; unwrap a single such layer so
	...
	if executable != "opercia" && !strings.HasSuffix(executable, "/opercia") {
		return false
	}
...
// like `OPERCIA_TOKEN=x opercia issue comment add ...` is still recognized.
