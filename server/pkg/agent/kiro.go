...
func (b *kiroBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// (goal_complete or `operica issue comment add`). We record the status
	...
	c := &hermesClient{
		...
		onMessage: func(msg Message) {
			...
			if msg.Type == MessageToolUse {
				...
				// Recognize `operica issue comment add` by its command
...
// `operica issue comment add` invocation. It keys purely off the command
...
func isKiroIssueCommentAddCommand(command string) bool {
	...
	// `sh -c "operica issue comment add ..."`; unwrap a single such layer so
	...
	if executable != "operica" && !strings.HasSuffix(executable, "/operica") {
		return false
	}
...
// like `OPERICA_TOKEN=x operica issue comment add ...` is still recognized.
