...
const (
	opericaCodexMcpBeginMarker = "# BEGIN operica-managed mcp_servers (do not edit; regenerated by daemon)"
	opericaCodexMcpEndMarker   = "# END operica-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(opericaCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(opericaCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = opericaCodexMcpBeginMarker + "\n" + opericaCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(opericaCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(opericaCodexMcpEndMarker)
	sb.WriteString("\n")
...
func normalizeCodexMcpServerConfig(server map[string]any) map[string]any {
	...
	for k, v := range server {
		switch {
		case isOpericaMcpSelectorKey(k):
			continue
		case k == "type":
			continue
		case k == "headers":
...
func isOpericaMcpSelectorKey(k string) bool {
	switch k {
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "operica-agent-sdk",
				"title":   "Operica Agent SDK",
				"version": "0.2.0",
			},
