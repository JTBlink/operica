...
const (
	operciaCodexMcpBeginMarker = "# BEGIN opercia-managed mcp_servers (do not edit; regenerated by daemon)"
	operciaCodexMcpEndMarker   = "# END opercia-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(operciaCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(operciaCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = operciaCodexMcpBeginMarker + "\n" + operciaCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(operciaCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(operciaCodexMcpEndMarker)
	sb.WriteString("\n")
...
func normalizeCodexMcpServerConfig(server map[string]any) map[string]any {
	...
	for k, v := range server {
		switch {
		case isOperciaMcpSelectorKey(k):
			continue
		case k == "type":
			continue
		case k == "headers":
...
func isOperciaMcpSelectorKey(k string) bool {
	switch k {
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "opercia-agent-sdk",
				"title":   "Opercia Agent SDK",
				"version": "0.2.0",
			},
