...
func TestOpencodeExecuteSendsPromptOnStdinNotArgv(t *testing.T) {
	...
	prompt := "OPERCIA_AGENT_BUILDER_INPUT\n" +
		"{\n" +
	...
	for _, arg := range argv {
		for _, needle := range []string{"OPERCIA_AGENT_BUILDER_INPUT", "user_request", "-X", "inspect local"} {
...
func TestOpencodeExecutePromptExceedingWindowsCommandLineLimit(t *testing.T) {
	...
	prompt := strings.Repeat("opercia opencode workspace skill catalogue 0123456789\n", 800)
	if len(prompt) <= 32767 {
...
func TestOpencodeExecuteLargePromptDoesNotDeadlock(t *testing.T) {
	...
	prompt := strings.Repeat("opercia opencode stdin payload 0123456789\n", 16_384)
