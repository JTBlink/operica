...
func TestBuildPiArgsIgnoresSystemPrompt(t *testing.T) {
	args := buildPiArgs("/tmp/s.jsonl", ExecOptions{
		SystemPrompt: "the entire operica runtime brief",
	}, slog.Default())
	...
	for _, a := range args {
		...
		if a == "the entire operica runtime brief" {
...
func TestStripPiToolCallMarkup(t *testing.T) {
	tests := map[string]string{
		...
		`before response:bash{command:<|"|>operica issue comment list issue-id --all --output json<|"|>} after`: "before  after",
