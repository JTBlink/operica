...
func TestBuildPiArgsIgnoresSystemPrompt(t *testing.T) {
	args := buildPiArgs("/tmp/s.jsonl", ExecOptions{
		SystemPrompt: "the entire opercia runtime brief",
	}, slog.Default())
	...
	for _, a := range args {
		...
		if a == "the entire opercia runtime brief" {
...
func TestStripPiToolCallMarkup(t *testing.T) {
	tests := map[string]string{
		...
		`before response:bash{command:<|"|>opercia issue comment list issue-id --all --output json<|"|>} after`: "before  after",
