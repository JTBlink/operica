...
not import any other Opercia package, so it can be reused by other services or
...
link, err := client.CreateLink(ctx, composio.CreateLinkRequest{
    ...
    UserID:       operciaUserID.String(), // your own user id
    CallbackURL:  "https://opercia.ai/api/integrations/composio/callback",
})
...
- **Standalone.** Zero coupling to Opercia internals — depend on this package
