...
not import any other Operica package, so it can be reused by other services or
...
link, err := client.CreateLink(ctx, composio.CreateLinkRequest{
    ...
    UserID:       opericaUserID.String(), // your own user id
    CallbackURL:  "https://operica.ai/api/integrations/composio/callback",
})
...
- **Standalone.** Zero coupling to Operica internals — depend on this package
