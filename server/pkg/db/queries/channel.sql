...
-- re-connecting it — even to represent a DIFFERENT agent in the SAME Operica
...
-- the conflict update to the SAME Operica workspace: a team already owned by a
...
-- old catch-all "connected to a different Operica workspace". Meant to be read
...
-- to a Operica user. The old composite member-FK is gone, so this no
...
-- before calling. ON CONFLICT DO UPDATE is still gated on operica_user_id
...
INSERT INTO channel_user_binding (
    workspace_id, operica_user_id, installation_id,
...
ON CONFLICT (installation_id, channel_user_id) DO UPDATE SET
    ...
    bound_at = now()
WHERE channel_user_binding.operica_user_id = EXCLUDED.operica_user_id
RETURNING *;

-- name: GetChannelUserBindingByUserID :one
-- The inbound identity lookup: does this platform user id map to a Operica
...
-- Outbound notification lookup: given a Operica member and a channel_type,
...
WHERE b.workspace_id = sqlc.arg('workspace_id')
  AND b.operica_user_id = sqlc.arg('operica_user_id')
...
-- bound to ANOTHER installation in the SAME Operica workspace + SAME Slack team,
...
-- different Operica workspaces, and a user may hold different Operica accounts in
...
DELETE FROM channel_user_binding
WHERE workspace_id = $1 AND operica_user_id = $2;
