...
-- re-connecting it — even to represent a DIFFERENT agent in the SAME Opercia
...
-- the conflict update to the SAME Opercia workspace: a team already owned by a
...
-- old catch-all "connected to a different Opercia workspace". Meant to be read
...
-- to a Opercia user. The old composite member-FK is gone, so this no
...
-- before calling. ON CONFLICT DO UPDATE is still gated on opercia_user_id
...
INSERT INTO channel_user_binding (
    workspace_id, opercia_user_id, installation_id,
...
ON CONFLICT (installation_id, channel_user_id) DO UPDATE SET
    ...
    bound_at = now()
WHERE channel_user_binding.opercia_user_id = EXCLUDED.opercia_user_id
RETURNING *;

-- name: GetChannelUserBindingByUserID :one
-- The inbound identity lookup: does this platform user id map to a Opercia
...
-- Outbound notification lookup: given a Opercia member and a channel_type,
...
WHERE b.workspace_id = sqlc.arg('workspace_id')
  AND b.opercia_user_id = sqlc.arg('opercia_user_id')
...
-- bound to ANOTHER installation in the SAME Opercia workspace + SAME Slack team,
...
-- different Opercia workspaces, and a user may hold different Opercia accounts in
...
DELETE FROM channel_user_binding
WHERE workspace_id = $1 AND opercia_user_id = $2;
