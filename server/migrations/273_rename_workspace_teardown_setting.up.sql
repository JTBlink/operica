-- Trigger definitions are stored in PostgreSQL when they are created. Recreate
-- the teardown guards so databases upgraded from earlier releases use the
-- current product-prefixed transaction setting.
...
FOR EACH ROW
WHEN (current_setting('operica.workspace_teardown', true) IS DISTINCT FROM 'on')
