...
-- lark_installation
-- =====================
-- One row per (workspace, agent) — each Operica Agent owns at most one
...
-- lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a Operica user, per-installation. open_id is
...
--
--   2. The composite FK on (workspace_id, operica_user_id) targets
--      member(workspace_id, user_id) with ON DELETE CASCADE, so when a
--      Operica user is removed from the workspace the stale Lark
...
-- =====================
-- One Lark chat (`chat_id`) ↔ one Operica `chat_session`. The Lark side
...
-- =====================
-- Maps a Operica task (or session bootstrap "thinking…" card) to the
...
-- click here" flow that links a Lark `open_id` to a Operica user. The
