...
-- lark_installation
-- =====================
-- One row per (workspace, agent) — each Opercia Agent owns at most one
...
-- lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a Opercia user, per-installation. open_id is
...
--
--   2. The composite FK on (workspace_id, opercia_user_id) targets
--      member(workspace_id, user_id) with ON DELETE CASCADE, so when a
--      Opercia user is removed from the workspace the stale Lark
...
-- =====================
-- One Lark chat (`chat_id`) ↔ one Opercia `chat_session`. The Lark side
...
-- =====================
-- Maps a Opercia task (or session bootstrap "thinking…" card) to the
...
-- click here" flow that links a Lark `open_id` to a Opercia user. The
