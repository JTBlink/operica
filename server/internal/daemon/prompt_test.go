...
func TestBuildQuickCreatePromptRules(t *testing.T) {
	...
	mustContain := []string{
		...
		// auto-subscribe path fires (operica issue create has no --subscriber flag)
		"CC exception",
		...
		// fixed prefix.
		"operica issue create --output json",
		"JSON response",
		"identifier",
...
func TestBuildChatPromptAttachmentIDsCanBeBoundToCreatedIssues(t *testing.T) {
	...
	for _, want := range []string{
		...
		"operica attachment download <id>",
		"--attachment-id <id>",
	} {
		if !strings.Contains(out, want) {
...
func TestBuildChatPromptChannelAwareness(t *testing.T) {
	t.Run("slack-backed prompt teaches both read commands", func(t *testing.T) {
		...
		for _, want := range []string{"Slack", "NOT in Operica", "operica chat history", "operica chat thread", "Do NOT narrate"} {
			if !strings.Contains(out, want) {
	...
	t.Run("top-level mention starts with history", func(t *testing.T) {
		...
		if !strings.Contains(out, "top level: start with `operica chat history`") {
	...
	t.Run("in-thread mention starts with thread", func(t *testing.T) {
		...
		if !strings.Contains(out, "inside a thread: start with `operica chat thread`") {
	...
	t.Run("web-only session has no channel block", func(t *testing.T) {
		...
		if strings.Contains(out, "operica chat history") {
...
//     Any IM reply leaves Operica, where the upload has nothing to bind to.
...
func TestBuildChatPromptTwoLayerChannelPolicy(t *testing.T) {
	...
	// `operica attachment upload` on purpose — to state that it does not apply
	...
	const uploadGuidance = "run `operica attachment upload <local-path>`"
	const historyGuidance = "operica chat history"
...
func TestBuildChatPromptFeishuIgnoresChatInThread(t *testing.T) {
	...
	for _, unwanted := range []string{"operica chat thread", "operica chat history"} {
...
func TestBuildPromptDefaultScansRootsFirst(t *testing.T) {
	...
	for _, s := range []string{
		"operica issue comment list issue-default-1 --roots-only --summary --compact --output json",
		"--since",
	} {
		if !strings.Contains(out, s) {
	...
	if strings.Contains(out, "operica issue comment list issue-default-1 --output json") {
...
func TestBuildPromptNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "operica issue comment list "+issueID+" --thread thread-root-1 --since "+since+" --compact --output json") {
	...
	if strings.Contains(out, "operica issue comment list "+issueID+" --since "+since+" --output json") {
...
func TestBuildPromptColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "operica issue comment list "+issueID+" --thread thread-root-1 --tail 30 --compact --output json") {
	...
	if strings.Contains(out, "operica issue comment list "+issueID+" --roots-only --summary --output json") {
...
func TestBuildCommentPromptCoalescedIDsOnlyFallback(t *testing.T) {
	...
	t.Run("with since anchor", func(t *testing.T) {
		...
		want := "operica issue comment list issue-fallback-1 --since 2026-08-03T06:00:00Z --compact --output json"
		if !strings.Contains(out, want) {
...
func assertBoundedIDOnlyFallback(t *testing.T, out string) {
	...
	for _, want := range []string{
		"operica issue comment list issue-fallback-1 --thread <comment-id> --tail 30 --compact --output json",
		"accepts a reply id",
		"Next reply cursor",
		"--before-id",
...
func TestBuildCommentPromptCrossThreadFansOutReplies(t *testing.T) {
	...
	if strings.Contains(out, "operica issue comment add") {
