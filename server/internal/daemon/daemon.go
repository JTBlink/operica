...
const (
	...
	repoCheckoutModeEnv      = "OPERCIA_REPO_CHECKOUT_MODE"
...
// `opercia repo checkout`. Under Codex's workspace-write sandbox a linked
...
func taskOperciaEnvironment(task Task, agentName, token, configRoot, workspacesRoot, serverURL string, healthPort, slot int, tempDir string) map[string]string {
	return map[string]string{
		"OPERCIA_TOKEN":        token,
		cli.TaskConfigRootEnv:  configRoot,
		TaskWorkspacesRootEnv:  workspacesRoot,
		"OPERCIA_SERVER_URL":   serverURL,
		"OPERCIA_DAEMON_PORT":  strconv.Itoa(healthPort),
		"OPERCIA_WORKSPACE_ID": task.WorkspaceID,
		"OPERCIA_AGENT_NAME":   agentName,
		"OPERCIA_AGENT_ID":     task.AgentID,
		"OPERCIA_TASK_ID":      task.ID,
		"OPERCIA_TASK_SLOT":    strconv.Itoa(slot),
		"TMPDIR":               tempDir,
		"TMP":                  tempDir,
		"TEMP":                 tempDir,
	}
...
type Daemon struct {
	...
	brewTarget     string      // "<prefix>/bin/opercia" when brewInstall and the prefix resolved
	...
	// reloadPendingReason explains why a confirmed opercia version change hasn't
...
//     (preserving the ~/.opercia/hooks exclusion and the login-shell fallback).
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	// fault can strip every OPERCIA_* env var from an agent subprocess; the
...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		...
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'opercia login --profile %s'", d.cfg.Profile)
		}
...
func (d *Daemon) appendProfileRuntimes(ctx context.Context, workspaceID string, runtimes *[]map[string]string, failedProfiles *[]map[string]string) string {
	...
	for _, profile := range resp.RuntimeProfiles {
		...
		// path override (MUL-3284, `opercia runtime profile set-path`) wins
...
// `opercia repo checkout` accepts project-only URLs without an extra round
...
// other API call is what surfaces a user-actionable "run opercia login"
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Opercia Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// /opt/homebrew/bin/opercia) so the restarted daemon picks up the new Cellar
...
func (d *Daemon) restartTargetBinary() (string, error) {
	...
	d.brewTargetOnce.Do(func() {
		...
		if brewPrefix := getBrewPrefix(); brewPrefix != "" {
			d.brewTarget = filepath.Join(brewPrefix, "bin", "opercia")
		} else if prefix := matchKnownBrewPrefix(newBin); prefix != "" {
			d.brewTarget = filepath.Join(prefix, "bin", "opercia")
		}
	})
	if d.brewInstall {
		if d.brewTarget != "" {
...
// release. Used by pollLoop to expose OPERCIA_TASK_SLOT to spawned tasks.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	// here would make OPERCIA_WORKSPACE_ID empty in the agent env, and the
	...
	// `opercia repo checkout` would reject project-only URLs that aren't also
	...
	// /opercia_workspaces/{ws}/{short-id}/workdir hit FileNotFoundError in
	...
	// up stale Opercia instructions (issue id, trigger comment id, reply
	...
	if env.LocalDirectory {
		defer func() {
			...
			// Excise the sidecar tree (.agent_context/, .opercia/,
	...
	// can call the Opercia API and the local daemon (e.g. `opercia repo checkout`).
	// OPERCIA_TASK_SLOT is allocated from the daemon-wide concurrency pool, not
	// per-agent. When one daemon hosts multiple agents, slots index shared
	// daemon-level resources such as GPUs.
	// OPERCIA_TOKEN is bound to (agent, task) by the server. Never fall back
	...
	agentEnv := taskOperciaEnvironment(task, agentName, agentToken, env.OperciaConfigRoot, d.cfg.WorkspacesRoot, d.cfg.ServerBaseURL, d.cfg.HealthPort, slot, taskTempDir)
	if checkoutMode := repoCheckoutModeFor(provider, runtime.GOOS); checkoutMode != "" {
		agentEnv[repoCheckoutModeEnv] = checkoutMode
	}
	if task.AutopilotRunID != "" {
		agentEnv["OPERCIA_AUTOPILOT_RUN_ID"] = task.AutopilotRunID
	}
	if task.AutopilotID != "" {
		agentEnv["OPERCIA_AUTOPILOT_ID"] = task.AutopilotID
	}
	// Quick-create marker — when set, the opercia CLI's `issue create`
	...
	if task.QuickCreatePrompt != "" {
		agentEnv["OPERCIA_QUICK_CREATE_TASK_ID"] = task.ID
		if len(task.QuickCreateAttachmentIDs) > 0 {
			if raw, err := json.Marshal(task.QuickCreateAttachmentIDs); err == nil {
				agentEnv["OPERCIA_QUICK_CREATE_ATTACHMENT_IDS"] = string(raw)
			} else {
				taskLog.Warn("quick-create attachment ids: marshal failed; skipping env injection", "error", err)
			}
		}
	}
	// Ensure the opercia CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// opercia binary so that `opercia` commands in the agent always resolve.
	...
	// user's existing state (MUL-5578). The Opercia CLI is the exception:
	// OPERCIA_TASK_CONFIG_ROOT above redirects its implicit profile lookup to
	...
	// then the daemon-wide OPERCIA_<PROVIDER>_MODEL env var. If
	...
	// `opercia issue status` / `opercia issue comment add`, leaving issues
	// stuck in `todo`.
	//
...
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID, codexHome string, msgSeq *atomic.Int32) (agent.Result, int32, error) {
	...
	// zero remains authoritative so OPERCIA_AGENT_IDLE_WATCHDOG=0 still disables
...
func ensureTaskTempDir(envRoot string, workspaceID string, taskID string) (string, error) {
	...
	dir, err := os.MkdirTemp(socketSafeTempBaseDir(), "opercia-task-")
	if err != nil {
		return "", err
	}
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "OPERCIA_") {
		return true
	}
	switch upper {
...
// and Opercia never copies API keys into task-managed files.
