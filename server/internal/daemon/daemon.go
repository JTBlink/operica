...
const (
	...
	repoCheckoutModeEnv      = "OPERICA_REPO_CHECKOUT_MODE"
...
// `operica repo checkout`. Under Codex's workspace-write sandbox a linked
...
func taskOpericaEnvironment(task Task, agentName, token, configRoot, workspacesRoot, serverURL string, healthPort, slot int, tempDir string) map[string]string {
	return map[string]string{
		"OPERICA_TOKEN":        token,
		cli.TaskConfigRootEnv:  configRoot,
		TaskWorkspacesRootEnv:  workspacesRoot,
		"OPERICA_SERVER_URL":   serverURL,
		"OPERICA_DAEMON_PORT":  strconv.Itoa(healthPort),
		"OPERICA_WORKSPACE_ID": task.WorkspaceID,
		"OPERICA_AGENT_NAME":   agentName,
		"OPERICA_AGENT_ID":     task.AgentID,
		"OPERICA_TASK_ID":      task.ID,
		"OPERICA_TASK_SLOT":    strconv.Itoa(slot),
		"TMPDIR":               tempDir,
		"TMP":                  tempDir,
		"TEMP":                 tempDir,
	}
...
type Daemon struct {
	...
	brewTarget     string      // "<prefix>/bin/operica" when brewInstall and the prefix resolved
	...
	// reloadPendingReason explains why a confirmed operica version change hasn't
...
//     (preserving the ~/.operica/hooks exclusion and the login-shell fallback).
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	// fault can strip every OPERICA_* env var from an agent subprocess; the
...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		...
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'operica login --profile %s'", d.cfg.Profile)
		}
...
func (d *Daemon) appendProfileRuntimes(ctx context.Context, workspaceID string, runtimes *[]map[string]string, failedProfiles *[]map[string]string) string {
	...
	for _, profile := range resp.RuntimeProfiles {
		...
		// path override (MUL-3284, `operica runtime profile set-path`) wins
...
// `operica repo checkout` accepts project-only URLs without an extra round
...
// other API call is what surfaces a user-actionable "run operica login"
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Operica Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// /opt/homebrew/bin/operica) so the restarted daemon picks up the new Cellar
...
func (d *Daemon) restartTargetBinary() (string, error) {
	...
	d.brewTargetOnce.Do(func() {
		...
		if brewPrefix := getBrewPrefix(); brewPrefix != "" {
			d.brewTarget = filepath.Join(brewPrefix, "bin", "operica")
		} else if prefix := matchKnownBrewPrefix(newBin); prefix != "" {
			d.brewTarget = filepath.Join(prefix, "bin", "operica")
		}
	})
	if d.brewInstall {
		if d.brewTarget != "" {
...
// release. Used by pollLoop to expose OPERICA_TASK_SLOT to spawned tasks.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	// here would make OPERICA_WORKSPACE_ID empty in the agent env, and the
	...
	// `operica repo checkout` would reject project-only URLs that aren't also
	...
	// /operica_workspaces/{ws}/{short-id}/workdir hit FileNotFoundError in
	...
	// up stale Operica instructions (issue id, trigger comment id, reply
	...
	if env.LocalDirectory {
		defer func() {
			...
			// Excise the sidecar tree (.agent_context/, .operica/,
	...
	// can call the Operica API and the local daemon (e.g. `operica repo checkout`).
	// OPERICA_TASK_SLOT is allocated from the daemon-wide concurrency pool, not
	// per-agent. When one daemon hosts multiple agents, slots index shared
	// daemon-level resources such as GPUs.
	// OPERICA_TOKEN is bound to (agent, task) by the server. Never fall back
	...
	agentEnv := taskOpericaEnvironment(task, agentName, agentToken, env.OpericaConfigRoot, d.cfg.WorkspacesRoot, d.cfg.ServerBaseURL, d.cfg.HealthPort, slot, taskTempDir)
	if checkoutMode := repoCheckoutModeFor(provider, runtime.GOOS); checkoutMode != "" {
		agentEnv[repoCheckoutModeEnv] = checkoutMode
	}
	if task.AutopilotRunID != "" {
		agentEnv["OPERICA_AUTOPILOT_RUN_ID"] = task.AutopilotRunID
	}
	if task.AutopilotID != "" {
		agentEnv["OPERICA_AUTOPILOT_ID"] = task.AutopilotID
	}
	// Quick-create marker — when set, the operica CLI's `issue create`
	...
	if task.QuickCreatePrompt != "" {
		agentEnv["OPERICA_QUICK_CREATE_TASK_ID"] = task.ID
		if len(task.QuickCreateAttachmentIDs) > 0 {
			if raw, err := json.Marshal(task.QuickCreateAttachmentIDs); err == nil {
				agentEnv["OPERICA_QUICK_CREATE_ATTACHMENT_IDS"] = string(raw)
			} else {
				taskLog.Warn("quick-create attachment ids: marshal failed; skipping env injection", "error", err)
			}
		}
	}
	// Ensure the operica CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// operica binary so that `operica` commands in the agent always resolve.
	...
	// user's existing state (MUL-5578). The Operica CLI is the exception:
	// OPERICA_TASK_CONFIG_ROOT above redirects its implicit profile lookup to
	...
	// then the daemon-wide OPERICA_<PROVIDER>_MODEL env var. If
	...
	// `operica issue status` / `operica issue comment add`, leaving issues
	// stuck in `todo`.
	//
...
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID, codexHome string, msgSeq *atomic.Int32) (agent.Result, int32, error) {
	...
	// zero remains authoritative so OPERICA_AGENT_IDLE_WATCHDOG=0 still disables
...
func ensureTaskTempDir(envRoot string, workspaceID string, taskID string) (string, error) {
	...
	dir, err := os.MkdirTemp(socketSafeTempBaseDir(), "operica-task-")
	if err != nil {
		return "", err
	}
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "OPERICA_") {
		return true
	}
	switch upper {
...
// and Operica never copies API keys into task-managed files.
