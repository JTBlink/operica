...
// ParseSelfVersion pulls the version out of `operica --version` output, whose
// first line is rendered by cmd/operica's version template:
//
//	operica 0.3.7 (commit: abc1234, built: 2026-07-29T10:00:00Z)
...
//
// Exported solely so cmd/operica can pin that contract from the producing side:
...
func ParseSelfVersion(raw string) string {
	...
	if fields := strings.Fields(line); len(fields) >= 2 && fields[0] == "operica" {
		return fields[1]
	}
	return line
}
...
// different operica binary". It runs two independent checks on one goroutine,
...
// (--no-auto-update / OPERICA_DAEMON_AUTO_UPDATE=false), when the server is
...
func (d *Daemon) autoUpdateLoop(ctx context.Context) {
	...
	if reloadEnabled {
		d.logger.Info("auto-reload: watching the operica binary on disk",
...
// trySelfReload restarts the daemon when the operica binary on disk no longer
// reports the version compiled into this process.
//
// This is the out-of-band half of self-update. `brew upgrade operica`, a
...
// has its own switch (--no-auto-reload / OPERICA_DAEMON_AUTO_RELOAD=false /
// disable_auto_reload) rather than riding on OPERICA_DAEMON_AUTO_UPDATE.
//
...
func (d *Daemon) trySelfReload(ctx context.Context) {
	...
	reason := fmt.Sprintf("operica binary on disk reports %s, running %s", onDisk, d.cfg.CLIVersion)
...
// setReloadPending records that a operica version change is confirmed but the
