...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "opercia login") {
		t.Fatalf("401 warning must tell the user to run 'opercia login', got: %s", out)
	}
}
...
// and the operator would never see a "run opercia login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "opercia login") {
		t.Fatalf("expected the actionable 'run opercia login' hint in the WARN, got: %s", out)
	}
}
