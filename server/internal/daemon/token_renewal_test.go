...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "operica login") {
		t.Fatalf("401 warning must tell the user to run 'operica login', got: %s", out)
	}
}
...
// and the operator would never see a "run operica login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "operica login") {
		t.Fatalf("expected the actionable 'run operica login' hint in the WARN, got: %s", out)
	}
}
