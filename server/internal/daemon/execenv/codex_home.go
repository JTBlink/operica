...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// `missing field path` and bails out of `thread/start`. Operica writes the
	...
	// so cross-task and cross-workspace context leaks (operica#3130) cannot
...
// enumerated by a plain `codex` run, keeping Operica task history out of the
// user's own thread list.
const codexSessionStoreRoot = "operica-sessions"
...
// shared home's operica-sessions root that have not been touched within
