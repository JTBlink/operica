...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// `missing field path` and bails out of `thread/start`. Opercia writes the
	...
	// so cross-task and cross-workspace context leaks (opercia#3130) cannot
...
// enumerated by a plain `codex` run, keeping Opercia task history out of the
// user's own thread list.
const codexSessionStoreRoot = "opercia-sessions"
...
// shared home's opercia-sessions root that have not been touched within
