...
func TestCodexShellEnvAllowlistUsesExactTaskAndSafeInheritedNames(t *testing.T) {
	...
	inherited := []string{
		...
		"OPERICA_SERVER_URL=https://wrong.example",
	}
...
set = { FOO = "bar", OPERICA_TOKEN = "stale" }
...
`,
			...
			removed: []string{"legal quoted table", `FOO = "bar"`, `OPERICA_TOKEN = "stale"`},
		},
		{
...
`,
	...
	includeOnly := []string{"CUSTOM_FLAG", "OPERICA_SERVER_URL", "OPERICA_TOKEN", "PATH", "SystemRoot"}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(s, `"OPERICA_*"`) {
				t.Fatalf("managed policy must not use a OPERICA_* wildcard:\n%s", s)
			}
...
func TestEnsureCodexShellEnvPolicyConfigRejectsInvalidInputWithoutWriting(t *testing.T) {
	...
	if err := EnsureCodexShellEnvPolicyConfig(configPath, []string{"OPERICA_TOKEN", "PATH"}, testLogger()); err == nil {
...
func TestEnsureCodexShellEnvPolicyConfigIsIdempotent(t *testing.T) {
	t.Parallel()
	configPath := filepath.Join(t.TempDir(), "config.toml")
	includeOnly := []string{"OPERICA_SERVER_URL", "OPERICA_TOKEN", "PATH"}
	...
	s := string(data)
	assertValidToml(t, s)
	if n := strings.Count(s, opericaShellEnvBeginMarker); n != 1 {
