...
func TestCodexShellEnvAllowlistUsesExactTaskAndSafeInheritedNames(t *testing.T) {
	...
	inherited := []string{
		...
		"OPERCIA_SERVER_URL=https://wrong.example",
	}
...
set = { FOO = "bar", OPERCIA_TOKEN = "stale" }
...
`,
			...
			removed: []string{"legal quoted table", `FOO = "bar"`, `OPERCIA_TOKEN = "stale"`},
		},
		{
...
`,
	...
	includeOnly := []string{"CUSTOM_FLAG", "OPERCIA_SERVER_URL", "OPERCIA_TOKEN", "PATH", "SystemRoot"}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(s, `"OPERCIA_*"`) {
				t.Fatalf("managed policy must not use a OPERCIA_* wildcard:\n%s", s)
			}
...
func TestEnsureCodexShellEnvPolicyConfigRejectsInvalidInputWithoutWriting(t *testing.T) {
	...
	if err := EnsureCodexShellEnvPolicyConfig(configPath, []string{"OPERCIA_TOKEN", "PATH"}, testLogger()); err == nil {
...
func TestEnsureCodexShellEnvPolicyConfigIsIdempotent(t *testing.T) {
	t.Parallel()
	configPath := filepath.Join(t.TempDir(), "config.toml")
	includeOnly := []string{"OPERCIA_SERVER_URL", "OPERCIA_TOKEN", "PATH"}
	...
	s := string(data)
	assertValidToml(t, s)
	if n := strings.Count(s, operciaShellEnvBeginMarker); n != 1 {
