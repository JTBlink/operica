...
const sidecarManifestFile = ".opercia_sidecar_manifest.json"
...
//     so the agent still discovers the Opercia skill, just under a
//     different directory name.
//   - For Opercia-only namespaces (.agent_context/issue_context.md,
//     .opercia/project/resources.json) the caller swallows the error
...
// down a Opercia skill without colliding with a user-installed skill
...
// collision we append `-opercia`, then `-opercia-2`, `-opercia-3`,
...
// baseSlug: the bare slug first, then `-opercia`, then numbered variants.
//
...
func skillSlugCandidate(baseSlug string, attempt int) string {
	...
	case attempt <= 0:
		return baseSlug
	case attempt == 1:
		return baseSlug + "-opercia"
	default:
		return fmt.Sprintf("%s-opercia-%d", baseSlug, attempt)
	}
}
...
// (.agent_context/, .opercia/, .claude/skills/, .github/skills/,
...
// issue-review-opercia. A managed skill directory is platform-owned — the
