...
const sidecarManifestFile = ".operica_sidecar_manifest.json"
...
//     so the agent still discovers the Operica skill, just under a
//     different directory name.
//   - For Operica-only namespaces (.agent_context/issue_context.md,
//     .operica/project/resources.json) the caller swallows the error
...
// down a Operica skill without colliding with a user-installed skill
...
// collision we append `-operica`, then `-operica-2`, `-operica-3`,
...
// baseSlug: the bare slug first, then `-operica`, then numbered variants.
//
...
func skillSlugCandidate(baseSlug string, attempt int) string {
	...
	case attempt <= 0:
		return baseSlug
	case attempt == 1:
		return baseSlug + "-operica"
	default:
		return fmt.Sprintf("%s-operica-%d", baseSlug, attempt)
	}
}
...
// (.agent_context/, .operica/, .claude/skills/, .github/skills/,
...
// issue-review-operica. A managed skill directory is platform-owned — the
