...
func writeHeader(b *strings.Builder) {
	b.WriteString("# Opercia Agent Runtime\n\n")
	b.WriteString("You are a coding agent in the Opercia platform. Use the `opercia` CLI to interact with the platform.\n\n")
}
...
func BuildTaskInitiatorBlock(initiatorType, initiatorName, initiatorEmail string) string {
	...
	b.WriteString("The initiator — not the runtime owner — is who you are answering: apply any per-person privacy or access rules your instructions define. Your Opercia credentials stay scoped to the runtime owner, and initiator attribution does not change what you may read or write; do not assume the initiator can see everything you can.\n\n")
	return b.String()
}
...
// preserved: each `opercia issue …` command name, all three `comment add`
...
// create/update tasks" intro phrase, and `opercia issue comment add
// --help`.
//
...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Prefer `--output json` for structured data. The default brief lists only the core agent loop and common issue create/update tasks; for everything else run `opercia --help` or `opercia <command> --help`.\n\n")
	b.WriteString("### Core\n")
	b.WriteString("- `opercia issue get <id> --output json` — full issue.\n")
	b.WriteString("- `opercia issue comment list <issue-id> [--roots-only] [--summary] [--thread <comment-id> [--tail N] | --recent N] [--since <RFC3339>] --output json` — thread-aware comment reads. Bound a wide read with `--roots-only --summary` (roots plus `reply_count` / `last_activity_at`, clipped bodies); bound a deep one with `--thread <id> --tail N`; add `--compact` to any JSON read to drop echoed/null/bookkeeping fields. Careful with `--recent N`: it caps THREADS, not comments, and can return the whole history on a small issue. Resolved-thread folding, paging cursors, and full flag semantics: `--help`.\n")
	b.WriteString("- `opercia issue create --title \"...\" [--description-file <path>] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <YYYY-MM-DD>] [--attachment <path>]` — create an issue. For agent-authored long descriptions prefer `--description-file <path>` (heredoc stdin can swallow trailing flags, #4182). Write that file inside your working directory (e.g. `./description.md`), never `/tmp` or shared paths — same workdir rule as `## Comment Formatting`.\n")
	b.WriteString("- `opercia issue update <id> [--title X] [--description-file <path>] [--priority X] [--status X] [--assignee X] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <YYYY-MM-DD>]` — update fields; pass `--parent \"\"` to clear parent.\n")
	b.WriteString("- `opercia issue status <id> <status>` — flip status (todo / in_progress / in_review / done / blocked / backlog / cancelled).\n")
	b.WriteString("- `opercia issue children <id> [--output json]` — list a parent's sub-issues grouped by stage.\n")
	b.WriteString("- `opercia issue comment add <issue-id> [--content \"...\" | --content-file <path> | --content-stdin] [--parent <comment-id>] [--attachment <path>]` — post a comment. Agent-authored bodies MUST use `--content-file`; see `## Comment Formatting` for why. `opercia issue comment add --help` for full flags.\n")
	b.WriteString("- `opercia issue metadata list <issue-id> [--output json]` — list KV metadata.\n")
	b.WriteString("- `opercia issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]` — pin or overwrite a key.\n")
	b.WriteString("- `opercia issue metadata delete <issue-id> --key <k>` — remove a key.\n")
	b.WriteString("- `opercia repo checkout <url> [--ref <branch-or-sha>]` — repository checkout on a dedicated branch.\n\n")
	...
	if ctx.IsSquadLeader {
		...
		b.WriteString("- `opercia squad member set-role <squad-id> --member-id <id> --member-type <agent|member> --role <role> [--output json]` — change role in place (use this instead of remove+add).\n\n")
	}
}
...
// every CLI other than `opercia issue create`, so listing more would just
// tempt the model to bend the guardrail.
func writeAvailableCommandsQuickCreate(b *strings.Builder) {
	b.WriteString("## Available Commands\n\n")
	b.WriteString("**Use `--output json` for structured data.** For anything beyond `issue create`, run `opercia --help` or `opercia <command> --help`.\n\n")
	b.WriteString("### Core\n")
...
func writeRepositories(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Available in this workspace — `opercia repo checkout <url> [--ref <branch-or-sha>]` to fetch (creates a repository checkout on a dedicated branch).\n\n")
...
func writeProjectContext(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if len(ctx.ProjectResources) > 0 {
		b.WriteString("Project resources (also written to `.opercia/project/resources.json`):\n\n")
		for _, r := range ctx.ProjectResources {
			fmt.Fprintf(b, "- %s\n", formatProjectResource(r))
		}
		b.WriteString("\nResources are pointers — open them only when relevant to the task. ")
		b.WriteString("For `github_repo` resources, use `opercia repo checkout <url>` to fetch the code. Add `--ref <branch-or-sha>` when a task or handoff names an exact revision.\n\n")
	} else {
...
func writeIssueMetadata(b *strings.Builder) {
	...
	b.WriteString("- **Write on exit.** Only what a future run will actually re-read — short values, never secrets or long content. Overwrite or `opercia issue metadata delete` stale keys. Full write discipline: the `opercia-working-on-issues` skill.\n\n")
}
...
//   - Slack: the conversation lives in the channel and `opercia chat history` /
//     `opercia chat thread` can fetch it — see buildChatPrompt, which hands the
...
//     in the provider session, and Opercia ships no history reader for Feishu
...
func writeWorkflowChat(b *strings.Builder) {
	...
	b.WriteString("- You have full access to the `opercia` CLI to look up issues, workspace info, members, agents, etc.\n")
	b.WriteString("- If asked about issues, use `opercia issue list --output json` or `opercia issue get <id> --output json`\n")
	b.WriteString("- If asked about the workspace, use `opercia workspace get --output json`\n")
	b.WriteString("- If asked to perform actions (create issues, update status, etc.), use the appropriate CLI commands\n")
	b.WriteString("- If the task requires code changes, use `opercia repo checkout <url>` to get the code first. Use `--ref <branch-or-sha>` when you need an exact revision\n")
...
func writeWorkflowQuickCreate(b *strings.Builder) {
	b.WriteString("**This task was triggered by quick-create.** There is NO existing Opercia issue. Follow the field and output rules in the user message you just received; ignore the default assignment-task workflow.\n\n")
	b.WriteString("Hard guardrails (apply even if the user message is missing):\n")
	b.WriteString("- Run exactly one `opercia issue create` invocation, then exit.\n")
	b.WriteString("- Do NOT call `opercia issue get`, `opercia issue status`, or `opercia issue comment add` for this task — there is no issue to query, transition, or comment on. The platform writes the user's success/failure inbox notification automatically based on whether `opercia issue create` succeeded.\n")
...
const AutopilotIssueCommandsGuard = "Do not run `opercia issue get`, `opercia issue comment add`, or `opercia issue status` for this run unless the autopilot instructions explicitly tell you to create or update an issue"
...
func writeWorkflowAutopilot(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("**This task was triggered by an Autopilot in run-only mode.** There is no assigned Opercia issue for this run.\n\n")
	...
	if ctx.AutopilotID != "" {
		fmt.Fprintf(b, "- Run `opercia autopilot get %s --output json` if you need the full autopilot configuration\n", ctx.AutopilotID)
	}
...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("1. Read the issue (`opercia issue get`) to understand the context.\n")
	b.WriteString("2. Read the metadata bag (`opercia issue metadata list`) — best-effort, empty `{}` and CLI failures are normal. What to look for: `## Issue Metadata`.\n")
	...
	if ctx.IsSquadLeader {
		b.WriteString("5. **Post your final results as a comment** (unless your outcome is `no_action` — in that case, calling `opercia squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action or saying you are exiting silently): post it with `opercia issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n")
	} else {
		b.WriteString("5. **Post your final results as a comment — this step is mandatory**: post it with `opercia issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). `## Output` states why this call is the only delivery channel.\n")
	}
	b.WriteString("6. Before exiting, pin or clear a metadata key via `opercia issue metadata set`/`delete` only if it clears the bar in `## Issue Metadata`. Most runs write nothing here — that is the expected outcome, not a gap. When in doubt, do not write.\n\n")
	...
	b.WriteString("- Before step 4, run `opercia issue status <issue-id> in_progress`.\n")
	if ctx.IsSquadLeader {
		...
		b.WriteString("- When done, run `opercia issue status <issue-id> in_review`.\n")
	}
	b.WriteString("- If blocked, run `opercia issue status <issue-id> blocked`, and post a comment explaining the blocker unless your Agent Identity forbids issue comments.\n\n")
...
// opercia-working-on-issues built-in skill: the semantics are only needed at
...
func writeSubIssueCreation(b *strings.Builder) {
	...
	b.WriteString("`--status todo` starts an agent-assigned child immediately; `--status backlog` parks it for later promotion; `--stage <N>` groups children into ordered stages. Before creating sub-issues, read the `opercia-working-on-issues` skill — it covers serial chains, promotion, and stage wake semantics.\n\n")
}
...
//
// The index itself stays because it is the one skill listing Opercia controls.
...
func writeAttachments(b *strings.Builder) {
	...
	b.WriteString("Fetch issue/comment attachments via the authenticated CLI (`opercia attachment --help`); never open Opercia resource URLs directly.\n")
...
// writeAlwaysUseCLI emits the "must go through the opercia CLI" guardrail
// (compressed).
func writeAlwaysUseCLI(b *strings.Builder) {
	b.WriteString("## Important: Always Use the `opercia` CLI\n\n")
	b.WriteString("Access Opercia platform resources only through the `opercia` CLI — never `curl` / `wget`. For anything the CLI doesn't cover, post a comment mentioning the workspace owner rather than working around it.\n\n")
}
...
func writeOutput(b *strings.Builder, kind taskKind, ctx TaskContextForEnv) {
	...
	case kindQuickCreate:
		b.WriteString("This is a quick-create task. There is NO existing issue to comment on. Your final stdout is captured automatically and the platform writes the user's success/failure inbox notification based on whether `opercia issue create` succeeded.\n\n")
		b.WriteString("- Do NOT call `opercia issue comment add` — the issue you just created has no conversation context for this run.\n")
		b.WriteString("- Print exactly one final line: `Created <identifier-or-id>: <title>` after a successful `opercia issue create`, using the created issue's `identifier` from JSON output (fall back to its `id`; never assume a workspace issue prefix such as `MUL-`).\n")
		...
		b.WriteString("**Delivering files here:** your stdout is text-only. A file that belongs to the new issue goes on the `opercia issue create` call itself via `--attachment <path>`; never put its path in the description or in your stdout line.\n")
	case kindChat:
		...
		// non-empty channel type means the reply leaves Opercia for an external
		...
		if ctx.ChatChannelType != "" {
			fmt.Fprintf(b, "**Delivering files here:** this %s conversation is text-only — Opercia cannot push a file you produced back into it. `opercia attachment upload` does NOT apply: it binds to a Opercia chat reply, which this is not. Say in words what you produced and where it can be obtained; never upload and then write as though the file arrived, and never link its local path.\n", ChannelDisplayName(ctx.ChatChannelType))
		} else {
			b.WriteString("**Delivering files here:** run `opercia attachment upload <local-path>` — it binds the file to your reply and it renders as an attachment card. That command is the ONLY way a file reaches the user; a path written into your reply text is not.\n")
		}
	default:
		if ctx.IsSquadLeader {
			b.WriteString("⚠️ **Final results MUST be delivered via `opercia issue comment add`** — unless your outcome is `no_action`. When you evaluate a trigger and decide no action is needed, calling `opercia squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment that announces no_action, acknowledges another agent, or says you are exiting silently — such comments are noise. For all other outcomes (`action`, `failed`), a comment is still mandatory.\n\n")
		} else {
			b.WriteString("⚠️ **Final results MUST be delivered via `opercia issue comment add`.** The user does NOT see your terminal output or run logs — only comments on the issue.\n\n")
		}
		...
		b.WriteString("**Delivering files here:** pass `--attachment <path>` to `opercia issue comment add` (repeatable) — the only way a screenshot or artifact reaches the reader.\n")
	}
	b.WriteString("\n")
