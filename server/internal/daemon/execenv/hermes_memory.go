...
// agent writes back) in `<HERMES_HOME>/memories/`. Operica has to take over
...
//
// This file makes memory an agent-scoped store Operica owns:
//
//	<operica profile dir>/hermes-state/<agent>/<hermes profile>/memories
//
...
// hermesMemoryStoreRoot is the directory under the daemon's Operica profile dir
...
// OpericaHermesTaskMemoryEnv is the rollback switch. Anything truthy (1, true,
...
const OpericaHermesTaskMemoryEnv = "OPERICA_HERMES_TASK_MEMORY"

// hermesTaskLocalMemoryForced reports whether the rollback switch is engaged.
func hermesTaskLocalMemoryForced() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv(OpericaHermesTaskMemoryEnv))) {
	case "1", "true", "yes", "on":
		return true
	}
	return false
...
// switch is engaged, there is no agent to key on, or the Operica profile dir
...
//
// daemonProfile namespaces by Operica profile for free: profile dirs are already
...
//     filesystems — the workspaces root and the Operica profile dir can be on
...
// the rollback path (OPERICA_HERMES_TASK_MEMORY), so it must actively replace a
