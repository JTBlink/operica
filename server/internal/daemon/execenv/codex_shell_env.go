...
// daemon-spawned Codex process can have OPERCIA_TOKEN while `opercia issue ...`
...
const (
	...
	operciaShellEnvBeginMarker = "# BEGIN opercia-managed shell-environment (do not edit; regenerated by daemon)"
	operciaShellEnvEndMarker   = "# END opercia-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(operciaShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(operciaShellEnvEndMarker) + `\n*`)
...
// custom_env. Inherited OPERCIA_* variables are always dropped because they
// belong to the daemon process, not necessarily to this task. Explicit
// OPERCIA_* values are safe to include because daemon.go blocklists that
...
func renderOperciaShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(operciaShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(operciaShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderOperciaShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
