...
// daemon-spawned Codex process can have OPERICA_TOKEN while `operica issue ...`
...
const (
	...
	opericaShellEnvBeginMarker = "# BEGIN operica-managed shell-environment (do not edit; regenerated by daemon)"
	opericaShellEnvEndMarker   = "# END operica-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(opericaShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(opericaShellEnvEndMarker) + `\n*`)
...
// custom_env. Inherited OPERICA_* variables are always dropped because they
// belong to the daemon process, not necessarily to this task. Explicit
// OPERICA_* values are safe to include because daemon.go blocklists that
...
func renderOpericaShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(opericaShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(opericaShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderOpericaShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
