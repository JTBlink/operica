...
// `no such host` errors when calling out (for example, `opercia issue get`
// hitting the Opercia API). See upstream issue openai/codex#10390.
//
...
// actually reach the Opercia API.
//
...
//     rejects non-safe mutation commands "by policy" — e.g. `opercia issue
...
//     danger-full-access so the Opercia CLI can reach the API.
...
// operciaManagedBeginMarker / operciaManagedEndMarker delimit the block the
...
const (
	operciaManagedBeginMarker = "# BEGIN opercia-managed (do not edit; regenerated by daemon)"
	operciaManagedEndMarker   = "# END opercia-managed"
)

// renderOperciaManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.opercia]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertOperciaManagedBlock), avoids both traps.
func renderOperciaManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(operciaManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(operciaManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(operciaManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(operciaManagedEndMarker) + `\n*`)

// upsertOperciaManagedBlock returns the config content with the opercia-managed
...
// `[permissions.opercia]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderOperciaManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertOperciaManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderOperciaManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the opercia-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertOperciaManagedBlock(existing, policy)
