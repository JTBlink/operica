...
// `no such host` errors when calling out (for example, `operica issue get`
// hitting the Operica API). See upstream issue openai/codex#10390.
//
...
// actually reach the Operica API.
//
...
//     rejects non-safe mutation commands "by policy" — e.g. `operica issue
...
//     danger-full-access so the Operica CLI can reach the API.
...
// opericaManagedBeginMarker / opericaManagedEndMarker delimit the block the
...
const (
	opericaManagedBeginMarker = "# BEGIN operica-managed (do not edit; regenerated by daemon)"
	opericaManagedEndMarker   = "# END operica-managed"
)

// renderOpericaManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.operica]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertOpericaManagedBlock), avoids both traps.
func renderOpericaManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(opericaManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(opericaManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(opericaManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(opericaManagedEndMarker) + `\n*`)

// upsertOpericaManagedBlock returns the config content with the operica-managed
...
// `[permissions.operica]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderOpericaManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertOpericaManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderOpericaManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the operica-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertOpericaManagedBlock(existing, policy)
