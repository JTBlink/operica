...
func BuildNewCommentsHint(issueID, triggerCommentID, triggerThreadID, newCommentsSince string, newCommentCount int) string {
	...
	if threadID != "" {
		return fmt.Sprintf(
			"%d new comment(s) on this issue since your last run — don't read them all blindly. "+
				...
				"`opercia issue comment list %s --thread %s --since %s --compact --output json` "+
	...
	return fmt.Sprintf(
		"%d new comment(s) on this issue since your last run. Catch up: "+
			"`opercia issue comment list %s --since %s --compact --output json`.\n\n",
...
func BuildResumedCommentsHint(issueID, triggerCommentID, triggerThreadID string) string {
	...
	return fmt.Sprintf(
		"You're resuming the prior session, and the triggering comment is already included above. "+
			...
			"`opercia issue comment list %s --thread %s --tail 30 --compact --output json`.\n\n",
		issueID, threadID,
	)
}
...
//     `?` before the bytes reach `opercia.exe` (#2198 Chinese, #2236
//     Chinese, #2376 Cyrillic).
//     2. On any host, when the model emits a multi-flag command (e.g.
//     `opercia issue create --title ... --assignee-id ... --project ...`)
	...
	if runtimeGOOS == "windows" {
		return fmt.Sprintf(
			lead+
				...
				"    opercia issue comment add %s --parent %s --content-file ./reply.md\n"+
...
func BuildMultiThreadCommentReplyInstructions(issueID string, targets []ThreadReplyTarget, squadLeader bool) string {
	...
	if squadLeader {
		lead = "This run coalesced comments from %d DISTINCT threads. **If your outcome is `no_action`, skip this ENTIRE fan-out block — post no replies at all and exit via `opercia squad activity` as your leader rules direct; everything below applies only otherwise.** Otherwise, post ONE reply per thread"
	}
	return fmt.Sprintf(
