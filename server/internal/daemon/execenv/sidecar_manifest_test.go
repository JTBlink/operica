...
// nothing related to Opercia before a task ran must contain nothing
// related to Opercia after the task finishes — no .agent_context/,
// no .claude/skills/, no .opercia/, no stub directories. The test
...
// must still exist and the Opercia-written sibling must be gone.
...
// round-trip to the .opercia/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .opercia/sidecar.txt and created
	// .opercia/ + .opercia/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .opercia/.
	managedDir := filepath.Join(workDir, ".opercia")
	...
	// .opercia still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-opercia/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Opercia would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Opercia skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Opercia
// sibling (which lives at `<slug>-opercia`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Opercia's version",
						Content:     "---\nname: issue-review\n---\n\nOpercia skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a Opercia-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.opercia/project/
// resources.json` — the other Opercia-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".opercia", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".opercia", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-opercia`, then `-opercia-2`,
// `-opercia-3`, … The PR-review concern is "Opercia skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-opercia`.
	...
	if slug != "issue-review-opercia" {
		t.Errorf("second allocation should bump to `-opercia`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-opercia") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-opercia"), 0o755); err != nil {
	...
	if slug != "issue-review-opercia-2" {
		t.Errorf("third allocation should be `-opercia-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-opercia-2") {
...
// `Issue Review` skill, the Opercia sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Opercia sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Opercia skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Opercia's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	operciaDir := filepath.Join(workDir, ".claude", "skills", "issue-review-opercia")
	if _, err := os.Stat(filepath.Join(operciaDir, "SKILL.md")); err != nil {
		t.Errorf("Opercia sibling skill should exist at %s: %v", operciaDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Opercia sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(operciaDir); !os.IsNotExist(err) {
		t.Errorf("Opercia sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
