...
// nothing related to Operica before a task ran must contain nothing
// related to Operica after the task finishes — no .agent_context/,
// no .claude/skills/, no .operica/, no stub directories. The test
...
// must still exist and the Operica-written sibling must be gone.
...
// round-trip to the .operica/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .operica/sidecar.txt and created
	// .operica/ + .operica/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .operica/.
	managedDir := filepath.Join(workDir, ".operica")
	...
	// .operica still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-operica/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Operica would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Operica skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Operica
// sibling (which lives at `<slug>-operica`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Operica's version",
						Content:     "---\nname: issue-review\n---\n\nOperica skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a Operica-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.operica/project/
// resources.json` — the other Operica-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".operica", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".operica", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-operica`, then `-operica-2`,
// `-operica-3`, … The PR-review concern is "Operica skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-operica`.
	...
	if slug != "issue-review-operica" {
		t.Errorf("second allocation should bump to `-operica`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-operica") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-operica"), 0o755); err != nil {
	...
	if slug != "issue-review-operica-2" {
		t.Errorf("third allocation should be `-operica-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-operica-2") {
...
// `Issue Review` skill, the Operica sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Operica sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Operica skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Operica's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	opericaDir := filepath.Join(workDir, ".claude", "skills", "issue-review-operica")
	if _, err := os.Stat(filepath.Join(opericaDir, "SKILL.md")); err != nil {
		t.Errorf("Operica sibling skill should exist at %s: %v", opericaDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Operica sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(opericaDir); !os.IsNotExist(err) {
		t.Errorf("Operica sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
