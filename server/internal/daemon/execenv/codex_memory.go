...
// — users cannot audit or edit the contents from any Operica UI.
//
// This conflicts with the Operica daemon's context model. Operica
...
//     unrelated host-project context into Operica tasks. The reproduction
//     in github.com/JTBlink/operica#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Operica issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Operica task
// (and accept the leak risk) can keep the feature enabled by setting
// `OPERICA_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Operica-owned, user-visible,
...
// OpericaCodexMemoryEnv is the env var users can set to keep Codex
...
const OpericaCodexMemoryEnv = "OPERICA_CODEX_MEMORY"

const (
	opericaMemoryFeatureBeginMarker = "# BEGIN operica-managed memory-feature (do not edit; regenerated by daemon)"
	opericaMemoryFeatureEndMarker   = "# END operica-managed memory-feature"
	opericaMemoryConfigBeginMarker  = "# BEGIN operica-managed memory-config (do not edit; regenerated by daemon)"
	opericaMemoryConfigEndMarker    = "# END operica-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(opericaMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(opericaMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(opericaMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(opericaMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(OpericaCodexMemoryEnv))
...
// renderOpericaMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderOpericaMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(opericaMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(opericaMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderOpericaMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderOpericaMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(opericaMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(opericaMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderOpericaMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderOpericaMemoryConfigBlock(true))
}
...
//
// When OPERICA_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per OPERICA_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderOpericaMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderOpericaMemoryConfigBlock(false))
	}
