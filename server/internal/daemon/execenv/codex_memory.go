...
// — users cannot audit or edit the contents from any Opercia UI.
//
// This conflicts with the Opercia daemon's context model. Opercia
...
//     unrelated host-project context into Opercia tasks. The reproduction
//     in github.com/opercia-ai/opercia#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Opercia issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Opercia task
// (and accept the leak risk) can keep the feature enabled by setting
// `OPERCIA_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Opercia-owned, user-visible,
...
// OperciaCodexMemoryEnv is the env var users can set to keep Codex
...
const OperciaCodexMemoryEnv = "OPERCIA_CODEX_MEMORY"

const (
	operciaMemoryFeatureBeginMarker = "# BEGIN opercia-managed memory-feature (do not edit; regenerated by daemon)"
	operciaMemoryFeatureEndMarker   = "# END opercia-managed memory-feature"
	operciaMemoryConfigBeginMarker  = "# BEGIN opercia-managed memory-config (do not edit; regenerated by daemon)"
	operciaMemoryConfigEndMarker    = "# END opercia-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(operciaMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(operciaMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(operciaMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(operciaMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(OperciaCodexMemoryEnv))
...
// renderOperciaMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderOperciaMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(operciaMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(operciaMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderOperciaMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderOperciaMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(operciaMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(operciaMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderOperciaMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderOperciaMemoryConfigBlock(true))
}
...
//
// When OPERCIA_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per OPERCIA_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderOperciaMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderOperciaMemoryConfigBlock(false))
	}
