...
const (
	runtimeMarkerBegin = "<!-- BEGIN OPERCIA-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END OPERCIA-RUNTIME -->"
...
// writeRuntimeConfigFile writes the Opercia runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the Opercia marker
...
//
//   - User content carries a stray `<!-- END OPERCIA-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the Opercia marker block from the runtime
...
// of Opercia runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a Opercia task would pick up
...
// agent about the Opercia runtime environment and available CLI tools.
//
