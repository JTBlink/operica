...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the Operica-managed brief
...
const (
	runtimeMarkerBegin = "<!-- BEGIN OPERICA-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END OPERICA-RUNTIME -->"
...
// writeRuntimeConfigFile writes the Operica runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the Operica marker
...
//
//   - User content carries a stray `<!-- END OPERICA-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the Operica marker block from the runtime
...
// of Operica runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a Operica task would pick up
...
// agent about the Operica runtime environment and available CLI tools.
//
