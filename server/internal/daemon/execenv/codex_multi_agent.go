...
// subagents. The Opercia daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Opercia runtime
...
//
// Users who explicitly want Codex native subagents inside a Opercia task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `OPERCIA_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// OperciaCodexMultiAgentEnv is the env var users can set to keep Codex
...
const OperciaCodexMultiAgentEnv = "OPERCIA_CODEX_MULTI_AGENT"

// operciaMultiAgentBeginMarker / operciaMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	operciaMultiAgentBeginMarker = "# BEGIN opercia-managed multi-agent (do not edit; regenerated by daemon)"
	operciaMultiAgentEndMarker   = "# END opercia-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(operciaMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(operciaMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(OperciaCodexMultiAgentEnv))
...
// renderOperciaMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderOperciaMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(operciaMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(operciaMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderOperciaMultiAgentBlock(true)
...
//
// When OPERCIA_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per OPERCIA_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderOperciaMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
