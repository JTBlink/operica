...
// subagents. The Operica daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Operica runtime
...
//
// Users who explicitly want Codex native subagents inside a Operica task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `OPERICA_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// OpericaCodexMultiAgentEnv is the env var users can set to keep Codex
...
const OpericaCodexMultiAgentEnv = "OPERICA_CODEX_MULTI_AGENT"

// opericaMultiAgentBeginMarker / opericaMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	opericaMultiAgentBeginMarker = "# BEGIN operica-managed multi-agent (do not edit; regenerated by daemon)"
	opericaMultiAgentEndMarker   = "# END operica-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(opericaMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(opericaMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(OpericaCodexMultiAgentEnv))
...
// renderOpericaMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderOpericaMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(opericaMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(opericaMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderOpericaMultiAgentBlock(true)
...
//
// When OPERICA_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per OPERICA_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderOpericaMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
