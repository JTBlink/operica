...
// sandbox strips all OPERCIA_* env vars before invoking `opercia`.
const TaskContextMarkerRelPath = ".opercia/daemon_task_context.json"
...
const TaskContextMarkerManagedBy = "opercia-daemon-task"
...
// {workspacesRoot}/.opercia/daemon_task_context.json.
//
// The per-workdir marker only protects `opercia` invocations whose cwd is
// inside the workdir, because the CLI discovers markers by walking *up* from
// cwd. A sandboxed subprocess that lost every OPERCIA_* env var and escaped
...
func writeTaskContextMarker(workDir string, ctx TaskContextForEnv, manifest *sidecarManifest) error {
	...
	if err := recordMkdirAll(dir, 0o755, manifest); err != nil {
		return fmt.Errorf("create .opercia dir: %w", err)
	}
...
// writeProjectResources writes .opercia/project/resources.json into the
...
//
// manifest, when non-nil, is populated with the .opercia/project chain
...
func writeProjectResources(workDir string, ctx TaskContextForEnv, manifest *sidecarManifest) error {
	...
	dir := filepath.Join(workDir, ".opercia", "project")
	if err := recordMkdirAll(dir, 0o755, manifest); err != nil {
		return err
	}
	...
	if err := recordWriteFile(filepath.Join(dir, "resources.json"), data, 0o644, manifest); err != nil {
		// .opercia/project/resources.json is Opercia-owned and a
...
//
// `name` is the one key Opercia must own. Runtimes disagree on which field
...
//
// When a Opercia skill's natural slug collides with a user-installed
// skill at the same path, we allocate a collision-free sibling slug
// (e.g. `issue-review-opercia`) and write there instead. Provider-native
...
// skill entirely (which would silently drop a Opercia skill the agent
// expects to see).
...
func renderIssueContext(provider string, ctx TaskContextForEnv) string {
	...
	fmt.Fprintf(&b, "Run `opercia issue get %s --output json` to fetch the full issue details.\n\n", ctx.IssueID)
...
func renderAutopilotContext(ctx TaskContextForEnv) string {
	...
	b.WriteString("## Quick Start\n\n")
	b.WriteString("This is a run-only autopilot task with no assigned issue. Do not run `opercia issue get` unless the autopilot instructions explicitly ask you to create or update an issue.\n\n")
	if ctx.AutopilotID != "" {
