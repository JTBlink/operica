...
// sandbox strips all OPERICA_* env vars before invoking `operica`.
const TaskContextMarkerRelPath = ".operica/daemon_task_context.json"
...
const TaskContextMarkerManagedBy = "operica-daemon-task"
...
// {workspacesRoot}/.operica/daemon_task_context.json.
//
// The per-workdir marker only protects `operica` invocations whose cwd is
// inside the workdir, because the CLI discovers markers by walking *up* from
// cwd. A sandboxed subprocess that lost every OPERICA_* env var and escaped
...
func writeTaskContextMarker(workDir string, ctx TaskContextForEnv, manifest *sidecarManifest) error {
	...
	if err := recordMkdirAll(dir, 0o755, manifest); err != nil {
		return fmt.Errorf("create .operica dir: %w", err)
	}
...
// writeProjectResources writes .operica/project/resources.json into the
...
//
// manifest, when non-nil, is populated with the .operica/project chain
...
func writeProjectResources(workDir string, ctx TaskContextForEnv, manifest *sidecarManifest) error {
	...
	dir := filepath.Join(workDir, ".operica", "project")
	if err := recordMkdirAll(dir, 0o755, manifest); err != nil {
		return err
	}
	...
	if err := recordWriteFile(filepath.Join(dir, "resources.json"), data, 0o644, manifest); err != nil {
		// .operica/project/resources.json is Operica-owned and a
...
//
// `name` is the one key Operica must own. Runtimes disagree on which field
...
//
// When a Operica skill's natural slug collides with a user-installed
// skill at the same path, we allocate a collision-free sibling slug
// (e.g. `issue-review-operica`) and write there instead. Provider-native
...
// skill entirely (which would silently drop a Operica skill the agent
// expects to see).
...
func renderIssueContext(provider string, ctx TaskContextForEnv) string {
	...
	fmt.Fprintf(&b, "Run `operica issue get %s --output json` to fetch the full issue details.\n\n", ctx.IssueID)
...
func renderAutopilotContext(ctx TaskContextForEnv) string {
	...
	b.WriteString("## Quick Start\n\n")
	b.WriteString("This is a run-only autopilot task with no assigned issue. Do not run `operica issue get` unless the autopilot instructions explicitly ask you to create or update an issue.\n\n")
	if ctx.AutopilotID != "" {
