...
This skill states WHAT a mention link does in the Opercia backend, traced to
...
at a project (see the opercia-projects-and-resources skill); everything else in
...
- a person → `opercia workspace member list --output json` → use `user_id`
- an agent → `opercia agent list --output json` → use `id`
- a squad  → `opercia squad list --output json` → use `id`
...
- **A hex-ish but wrong UUID.** A well-formed-looking UUID that no entity owns
  ...
  **So when you see `invocation_not_allowed`, check the UUID against the live
  ...
  `opercia squad member list <squad-id> --output json` returns the `member_id`
...
- **A private agent you cannot invoke:** blocked — the mention path gates on
  ...
  ungated — see the opercia-squads skill.)
...
Correct:
  1. `opercia workspace member list --output json`  → Alice's `user_id` = 7f3a…
