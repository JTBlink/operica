---
name: operica-autopilots
description: "Use when creating, updating, inspecting, triggering, or debugging a Operica autopilot (scheduled, webhook, or manual)."
...
```bash
operica autopilot list --output json
operica autopilot get <autopilot-id> --output json
operica autopilot runs <autopilot-id> --output json
```
...
- `create_issue` creates a Operica issue, making the run visible as issue state.
...
operica autopilot list --output json
operica autopilot get <autopilot-id> --output json
operica autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
operica autopilot update <autopilot-id> --status active|paused --output json
operica autopilot runs <autopilot-id> --output json
operica autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
operica autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
operica autopilot trigger <autopilot-id> --output json
operica autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
1. `operica autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `operica autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a squad, inspect the squad: `operica squad get <squad-id> --output json`; execution goes to the leader.
4. Inspect the target agent/runtime: `operica agent get <agent-id> --output json` and `operica runtime list --output json`.
