---
name: opercia-skill-importing
description: "Use when asked to import or install a specific skill into this Opercia workspace from a URL or slug. Not for choosing which skill the user needs; never treat a local installer such as `npx skills add` as the final install."
user-invocable: false
allowed-tools: Bash(opercia *)
---

# Importing skills into Opercia

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Opercia workspace.
...
A skill is installed for Opercia only when it exists in the current workspace's
...
```bash
opercia skill import --url <url> --output json              # hosted source
opercia skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the Opercia workspace DB, so Opercia cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `opercia skill import --url <url> --output json`:

```bash
opercia skill import --url clawhub.ai/owner/skill --output json
opercia skill import --url skills.sh/owner/repo/skill --output json
opercia skill import --url github.com/owner/repo --output json
opercia skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
opercia skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`opercia skill import --file <path> --output json` imports a skill from a local
...
After the final `opercia agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`opercia agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`opercia agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `opercia skill import --url <url>` is equivalent to
...
opercia skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
opercia skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
opercia skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
opercia skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
opercia skill get <skill-id> --output json
```
...
Incorrect (bypasses Opercia):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Opercia cannot manage it as a workspace skill.
...
```bash
opercia skill import --url https://skills.sh/owner/repo/skill --output json
```
