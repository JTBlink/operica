...
This skill states WHAT a mention link does in the Operica backend, traced to
...
at a project (see the operica-projects-and-resources skill); everything else in
...
- a person → `operica workspace member list --output json` → use `user_id`
- an agent → `operica agent list --output json` → use `id`
- a squad  → `operica squad list --output json` → use `id`
...
- **A hex-ish but wrong UUID.** A well-formed-looking UUID that no entity owns
  ...
  **So when you see `invocation_not_allowed`, check the UUID against the live
  ...
  `operica squad member list <squad-id> --output json` returns the `member_id`
...
- **A private agent you cannot invoke:** blocked — the mention path gates on
  ...
  ungated — see the operica-squads skill.)
...
Correct:
  1. `operica workspace member list --output json`  → Alice's `user_id` = 7f3a…
