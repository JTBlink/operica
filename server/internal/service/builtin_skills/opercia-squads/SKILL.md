---
name: opercia-squads
description: "Use when creating, inspecting, updating, assigning to, or debugging a Opercia squad, including how leader routing picks who runs."
...
opercia squad get <squad-id> --output json
opercia squad member list <squad-id> --output json
opercia issue comment list <issue-id> --roots-only --summary --output json
opercia issue comment list <issue-id> --thread <thread-id> --tail 30 --output json
```
...
```bash
opercia squad --help
opercia squad member --help
opercia issue update --help
...
A Opercia squad is a workspace routing and coordination object.
...
```bash
opercia squad list --output json
opercia squad get <squad-id> --output json
opercia squad create --name <name> --leader <agent-name-or-id> --output json
opercia squad update <squad-id> --instructions "<leader coordination policy>" --output json
opercia squad delete <squad-id>
```
...
```bash
opercia squad member list <squad-id> --output json
opercia squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
opercia squad member remove <squad-id> --member-id <id> --type agent|member
opercia squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
opercia squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
opercia issue get <issue-id> --output json
opercia issue update <issue-id> --help
opercia issue comment list <issue-id> --roots-only --summary --output json
opercia issue comment add <issue-id> --help
```
...
For squad leader tasks, Opercia appends a squad leader briefing to the leader
...
members carry no skills segment. Builtin `opercia-*` skills are not listed —
...
- recording squad activity with `opercia squad activity`;
...
- Getting the leader briefing does NOT imply status authority. A squad
  ...
  delegation rules yes, `opercia issue status` no.
