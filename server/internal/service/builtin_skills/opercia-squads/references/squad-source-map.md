...
This file records source evidence for `opercia-squads/SKILL.md`.
...
```text
server/cmd/opercia/cmd_squad.go
```
...
```bash
opercia squad list
opercia squad get <squad-id>
opercia squad create
opercia squad update <squad-id>
opercia squad delete <squad-id>
opercia squad activity <issue-id> <outcome>

opercia squad member list <squad-id>
opercia squad member add <squad-id>
opercia squad member remove <squad-id>
...
- agent member roster rows list assigned workspace skills via
  ...
  "no skills assigned"; builtin opercia-* skills are excluded and human
...
- parent status is not auto-advanced by the barrier: the system comment asks the
  ...
  `opercia issue status <parent-id> in_review`. That explicit ask is what lets a
