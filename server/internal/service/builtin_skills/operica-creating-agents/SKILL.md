---
name: operica-creating-agents
description: "Use when creating, inspecting, or debugging a Operica agent definition via the `operica agent` CLI or POST /api/agents. Not for assigning issues to agents that already exist, and not for runtime task prompts."
user-invocable: false
allowed-tools: Bash(operica *)
---

# Creating Operica agents

This is the contract for Operica's agent-creation path: what the create entry
...
```bash
operica agent get <agent-id> --output json      # full persisted agent record
operica agent skills list <agent-id> --output json   # current skill bindings
operica agent env get <agent-id> --output json  # plaintext env (agent owner or ws owner/admin; agents denied)
```
...
`POST /api/agents` (`operica agent create`). At task claim time the daemon
...
`operica agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
operica agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
operica agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
```bash
operica agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
operica agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`operica agent env get`). It is gated to the **agent's own human
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`operica agent env set`), which carries the same
...
```bash
operica agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
operica agent update <agent-id> --mcp-config-stdin --output json
operica agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
- `operica agent create` — inserts a new agent row.
- `operica agent copy` — inserts a new agent row (a fork of an existing agent);
  the source is left untouched.
- `operica agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `operica agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
