---
name: operica-working-on-issues
description: "Use when acting on a Operica issue beyond what the brief covers: PR linking vs close intent, reading a linked PR's real state, metadata keys, status-change side effects, sub-issue todo vs backlog."
user-invocable: false
allowed-tools: Bash(operica *), Bash(git *), Bash(gh *)
---

# Working on Operica issues
...
For building mention links, load `operica-mentioning` instead — not this skill.
...
This is the link that `operica issue pull-requests` reads back — but see the
...
**excluded from `operica issue pull-requests`** (and the issue's right-side PR
...
is to open or update a PR before posting the final Operica issue comment, unless
...
When a step depends on PR state, query Operica's link table — do not infer it
...
```bash
operica issue metadata set <issue-id> --key <key> --value <value>
operica issue metadata delete <issue-id> --key <stale-key>
```
...
- Read what exists before writing: `operica property list` shows the catalog;
  `operica issue property list <issue-id>` shows values set on the issue.
- Set values by property name and option name — the CLI translates to ids:

```bash
operica issue property set <issue-id> --name Environment --value staging
operica issue property set <issue-id> --name Platforms --value "iOS,Android"
operica issue property unset <issue-id> --name Environment
```
...
```bash
operica issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
operica issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
operica issue status <child-id> todo   # promote when the previous step is truly done
```
...
operica issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
operica issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
operica issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
operica issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
operica issue children <parent-id>             # sub-issues grouped by stage
operica issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
operica issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
operica issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
operica issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
operica issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
operica issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
