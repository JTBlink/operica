...
## `operica issue pull-requests` — read PR links from Operica

| Behavior | File:line | Drifted from |
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/operica/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/operica/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/operica/cmd_issue.go:522` | `:522` (unchanged) |
...
This is what `operica issue pull-requests` later reads back.
...
| `--stage` on `issue create` / `issue update` | `server/cmd/operica/cmd_issue.go:328,350` |
| `operica issue children <id>` (sub-issues grouped by stage) | `server/cmd/operica/cmd_issue.go:114,678`; route `GET /api/issues/{id}/children` → `ListChildIssues` |
...
comment explicitly asks for `operica issue status <parent-id> in_review` —
...
| Behavior | File:line |
|---|---|
| `operica issue metadata set <issue-id> --key --value [--type]` | `server/cmd/operica/cmd_issue_metadata.go:80,109-111` |
| `operica issue metadata delete <issue-id> --key` | `server/cmd/operica/cmd_issue_metadata.go:93,113` |
...
| Behavior | File:line |
|---|---|
| `operica property list/get/create/update/archive/unarchive` | `server/cmd/operica/cmd_property.go` |
| `operica issue property list/set/unset` (name→id translation) | `server/cmd/operica/cmd_property.go` (`encodeIssuePropertyValue`) |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/operica/cmd_issue.go
