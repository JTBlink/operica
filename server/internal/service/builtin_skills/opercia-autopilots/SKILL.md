---
name: opercia-autopilots
description: "Use when creating, updating, inspecting, triggering, or debugging a Opercia autopilot (scheduled, webhook, or manual)."
...
```bash
opercia autopilot list --output json
opercia autopilot get <autopilot-id> --output json
opercia autopilot runs <autopilot-id> --output json
```
...
- `create_issue` creates a Opercia issue, making the run visible as issue state.
...
opercia autopilot list --output json
opercia autopilot get <autopilot-id> --output json
opercia autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
opercia autopilot update <autopilot-id> --status active|paused --output json
opercia autopilot runs <autopilot-id> --output json
opercia autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
opercia autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
opercia autopilot trigger <autopilot-id> --output json
opercia autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
1. `opercia autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `opercia autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a squad, inspect the squad: `opercia squad get <squad-id> --output json`; execution goes to the leader.
4. Inspect the target agent/runtime: `opercia agent get <agent-id> --output json` and `opercia runtime list --output json`.
