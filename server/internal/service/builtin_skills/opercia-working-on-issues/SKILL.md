---
name: opercia-working-on-issues
description: "Use when acting on a Opercia issue beyond what the brief covers: PR linking vs close intent, reading a linked PR's real state, metadata keys, status-change side effects, sub-issue todo vs backlog."
user-invocable: false
allowed-tools: Bash(opercia *), Bash(git *), Bash(gh *)
---

# Working on Opercia issues
...
For building mention links, load `opercia-mentioning` instead — not this skill.
...
This is the link that `opercia issue pull-requests` reads back — but see the
...
**excluded from `opercia issue pull-requests`** (and the issue's right-side PR
...
is to open or update a PR before posting the final Opercia issue comment, unless
...
When a step depends on PR state, query Opercia's link table — do not infer it
...
```bash
opercia issue metadata set <issue-id> --key <key> --value <value>
opercia issue metadata delete <issue-id> --key <stale-key>
```
...
- Read what exists before writing: `opercia property list` shows the catalog;
  `opercia issue property list <issue-id>` shows values set on the issue.
- Set values by property name and option name — the CLI translates to ids:

```bash
opercia issue property set <issue-id> --name Environment --value staging
opercia issue property set <issue-id> --name Platforms --value "iOS,Android"
opercia issue property unset <issue-id> --name Environment
```
...
```bash
opercia issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
opercia issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
opercia issue status <child-id> todo   # promote when the previous step is truly done
```
...
opercia issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
opercia issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
opercia issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
opercia issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
opercia issue children <parent-id>             # sub-issues grouped by stage
opercia issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
opercia issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
opercia issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
opercia issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
opercia issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
opercia issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
