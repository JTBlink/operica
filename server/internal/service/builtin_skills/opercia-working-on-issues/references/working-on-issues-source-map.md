...
## `opercia issue pull-requests` — read PR links from Opercia

| Behavior | File:line | Drifted from |
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/opercia/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/opercia/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/opercia/cmd_issue.go:522` | `:522` (unchanged) |
...
This is what `opercia issue pull-requests` later reads back.
...
| `--stage` on `issue create` / `issue update` | `server/cmd/opercia/cmd_issue.go:328,350` |
| `opercia issue children <id>` (sub-issues grouped by stage) | `server/cmd/opercia/cmd_issue.go:114,678`; route `GET /api/issues/{id}/children` → `ListChildIssues` |
...
comment explicitly asks for `opercia issue status <parent-id> in_review` —
...
| Behavior | File:line |
|---|---|
| `opercia issue metadata set <issue-id> --key --value [--type]` | `server/cmd/opercia/cmd_issue_metadata.go:80,109-111` |
| `opercia issue metadata delete <issue-id> --key` | `server/cmd/opercia/cmd_issue_metadata.go:93,113` |
...
| Behavior | File:line |
|---|---|
| `opercia property list/get/create/update/archive/unarchive` | `server/cmd/opercia/cmd_property.go` |
| `opercia issue property list/set/unset` (name→id translation) | `server/cmd/opercia/cmd_property.go` (`encodeIssuePropertyValue`) |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/opercia/cmd_issue.go
