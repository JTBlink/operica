---
name: operica-skill-importing
description: "Use when asked to import or install a specific skill into this Operica workspace from a URL or slug. Not for choosing which skill the user needs; never treat a local installer such as `npx skills add` as the final install."
user-invocable: false
allowed-tools: Bash(operica *)
---

# Importing skills into Operica

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Operica workspace.
...
A skill is installed for Operica only when it exists in the current workspace's
...
```bash
operica skill import --url <url> --output json              # hosted source
operica skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the Operica workspace DB, so Operica cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `operica skill import --url <url> --output json`:

```bash
operica skill import --url clawhub.ai/owner/skill --output json
operica skill import --url skills.sh/owner/repo/skill --output json
operica skill import --url github.com/owner/repo --output json
operica skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
operica skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`operica skill import --file <path> --output json` imports a skill from a local
...
After the final `operica agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`operica agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`operica agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `operica skill import --url <url>` is equivalent to
...
operica skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
operica skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
operica skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
operica skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
operica skill get <skill-id> --output json
```
...
Incorrect (bypasses Operica):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Operica cannot manage it as a workspace skill.
...
```bash
operica skill import --url https://skills.sh/owner/repo/skill --output json
```
