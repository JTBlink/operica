---
name: opercia-creating-agents
description: "Use when creating, inspecting, or debugging a Opercia agent definition via the `opercia agent` CLI or POST /api/agents. Not for assigning issues to agents that already exist, and not for runtime task prompts."
user-invocable: false
allowed-tools: Bash(opercia *)
---

# Creating Opercia agents

This is the contract for Opercia's agent-creation path: what the create entry
...
```bash
opercia agent get <agent-id> --output json      # full persisted agent record
opercia agent skills list <agent-id> --output json   # current skill bindings
opercia agent env get <agent-id> --output json  # plaintext env (agent owner or ws owner/admin; agents denied)
```
...
`POST /api/agents` (`opercia agent create`). At task claim time the daemon
...
`opercia agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
opercia agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
opercia agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
```bash
opercia agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
opercia agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`opercia agent env get`). It is gated to the **agent's own human
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`opercia agent env set`), which carries the same
...
```bash
opercia agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
opercia agent update <agent-id> --mcp-config-stdin --output json
opercia agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
- `opercia agent create` — inserts a new agent row.
- `opercia agent copy` — inserts a new agent row (a fork of an existing agent);
  the source is left untouched.
- `opercia agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `opercia agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
