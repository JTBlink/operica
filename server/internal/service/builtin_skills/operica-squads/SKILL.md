---
name: operica-squads
description: "Use when creating, inspecting, updating, assigning to, or debugging a Operica squad, including how leader routing picks who runs."
...
operica squad get <squad-id> --output json
operica squad member list <squad-id> --output json
operica issue comment list <issue-id> --roots-only --summary --output json
operica issue comment list <issue-id> --thread <thread-id> --tail 30 --output json
```
...
```bash
operica squad --help
operica squad member --help
operica issue update --help
...
A Operica squad is a workspace routing and coordination object.
...
```bash
operica squad list --output json
operica squad get <squad-id> --output json
operica squad create --name <name> --leader <agent-name-or-id> --output json
operica squad update <squad-id> --instructions "<leader coordination policy>" --output json
operica squad delete <squad-id>
```
...
```bash
operica squad member list <squad-id> --output json
operica squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
operica squad member remove <squad-id> --member-id <id> --type agent|member
operica squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
operica squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
operica issue get <issue-id> --output json
operica issue update <issue-id> --help
operica issue comment list <issue-id> --roots-only --summary --output json
operica issue comment add <issue-id> --help
```
...
For squad leader tasks, Operica appends a squad leader briefing to the leader
...
members carry no skills segment. Builtin `operica-*` skills are not listed —
...
- recording squad activity with `operica squad activity`;
...
- Getting the leader briefing does NOT imply status authority. A squad
  ...
  delegation rules yes, `operica issue status` no.
