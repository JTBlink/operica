...
This file records source evidence for `operica-squads/SKILL.md`.
...
```text
server/cmd/operica/cmd_squad.go
```
...
```bash
operica squad list
operica squad get <squad-id>
operica squad create
operica squad update <squad-id>
operica squad delete <squad-id>
operica squad activity <issue-id> <outcome>

operica squad member list <squad-id>
operica squad member add <squad-id>
operica squad member remove <squad-id>
...
- agent member roster rows list assigned workspace skills via
  ...
  "no skills assigned"; builtin operica-* skills are excluded and human
...
- parent status is not auto-advanced by the barrier: the system comment asks the
  ...
  `operica issue status <parent-id> in_review`. That explicit ask is what lets a
