---
name: operica-projects-and-resources
description: "Use when creating, inspecting, updating, or debugging Operica projects and their resources (github_repo, local_directory)."
user-invocable: false
allowed-tools: Bash(operica *)
---

# Operica Projects and Resources
...
operica project get <project-id> --output json
...
A project groups work and carries durable resources. A resource is not just display metadata; it is context later injected into task briefs and `.operica/project/resources.json`.

A project's `description` is also durable context: when an issue (or a quick-create task) is bound to a project, the project description is injected into the agent's brief under `## Project Context` and written to `.operica/project/resources.json` as `project_description`. Use it for project-wide rules/context that should apply to every task in the project.
...
operica project list --output json
operica project get <project-id> --output json
operica project create --title "<title>" --repo <github-url> --output json
operica project create --title "<title>" --start-date 2026-03-01 --due-date 2026-03-31 --output json
operica project update <project-id> --title "<title>" --output json
operica project update <project-id> --due-date 2026-04-15 --output json
operica project update <project-id> --start-date "" --output json   # clear the start date
operica project status <project-id> in_progress --output json
operica project resource list <project-id> --output json
...
operica project resource add <project-id> --type local_directory --local-path <abs-path> --daemon-id <daemon-id> --output json
operica project resource update <project-id> <resource-id> --url <new-github-url> --output json
operica project resource update <project-id> <resource-id> --ref <branch-or-sha> --output json
operica project resource remove <project-id> <resource-id> --output json
```
...
Project resources are durable and affect future tasks. `operica repo checkout`
is task-local checkout state.

## Debugging wrong context

1. `operica project get <project-id> --output json`.
2. `operica project resource list <project-id> --output json`.
