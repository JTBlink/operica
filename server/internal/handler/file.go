...
type AttachmentResponse struct {
	...
	// OPERCIA_LOCAL_UPLOAD_BASE_URL), and otherwise prefixing
	// OPERCIA_PUBLIC_URL onto the stable per-attachment endpoint that the
...
//     proxies on every request, anchored on `OPERCIA_PUBLIC_URL` so the
//     persisted URL keeps working for clients that don't share the
//     document origin (Desktop / mobile webview).
//
//  3. Last-resort fallback (no `OPERCIA_PUBLIC_URL` configured): emit
