...
const (
	onboardingAssistantName = "Opercia Helper"
	onboardingIssueTitle    = "Start here: learn Opercia with Opercia Helper"
	onboardingAgentTemplate = "opercia_helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers Opercia questions and runs CLI operations."
...
// Opercia Helper agent created by this shim. Pre-v3 desktop submits a
...
const onboardingAssistantInstructions = `You are Opercia Helper, the built-in AI assistant for this Opercia workspace. Your role is to help any member use Opercia better — answer questions, give advice, and execute workspace operations on their behalf.
...
Your toolbox is the ` + "`opercia`" + ` CLI. It's already on your PATH and authenticated as the workspace owner.

Your full capability surface = whatever ` + "`opercia --help`" + ` shows. Run ` + "`opercia --help`" + ` first, then ` + "`opercia <command> --help`" + ` for any subcommand; use ` + "`--output json`" + ` for structured data. The CLI is your manifest — never invent commands or flags.
...
const onboardingIssueDescription = `Welcome to Opercia.

This is your guided first run. Opercia Helper is assigned to this issue and will help you try the core workflow:

1. Read Opercia Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention Opercia Helper when you want it to continue.
...
//
// Creates or reuses one "Opercia Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"Welcome to Opercia.",
		"",
		"Agents need a runtime before they can execute work. You can still use Opercia as a lightweight project-management workspace while you install one.",
		"",
		"## Try Opercia first",
		"",
		...
		"",
		"Full guide: https://opercia.ai/docs/install-agent-runtime",
		"",
		...
		"   which codex",
		"   codex --version",
		"5. Restart the Opercia daemon:",
		"   opercia daemon restart",
		...
		"",
		"When the runtime is connected, you can create Opercia Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"欢迎来到 Opercia。",
		"",
		"智能体需要先连上运行时才能执行工作。运行时还没准备好时，你也可以先把 Opercia 当作轻量项目管理工具体验起来。",
		"",
		"## 先体验项目管理功能",
		"",
		"运行时安装前，你可以先做这些事：",
		...
		"## 安装第一个 Agent 运行时",
		"",
		"完整文档：https://opercia.ai/docs/install-agent-runtime",
		"",
		"中文用户建议先装 Kimi CLI：",
		"",
		...
		"",
		"运行时连上后，你就可以创建 Opercia Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
