...
type Config struct {
	...
	// OPERICA_VCS_SECRET_KEY is set. It is the product boundary: the feature is
	// intended for self-hosted Operica only (where Operica and the Git instance
	...
	// OPERICA_VCS_INTEGRATION_ENABLED; the self-host compose defaults it on.
	VCSIntegrationEnabled bool
	// PublicURL is the absolute base URL the API is reachable at from the
	// public internet, with no trailing slash (e.g. "https://operica.ai").
	...
	// OPERICA_TRUSTED_PROXIES env var (comma-separated CIDRs, e.g.
	...
	//   - LLMAPIKey       -> OPERICA_LLM_API_KEY
	//   - LLMBaseURL       -> OPERICA_LLM_BASE_URL (OpenAI or any compatible gateway)
	//   - LLMDefaultModel  -> OPERICA_LLM_DEFAULT_MODEL (used when a request omits `model`)
	LLMAPIKey       string
	LLMBaseURL      string
...
type Handler struct {
	...
	// (OPERICA_LARK_SECRET_KEY) is unset; the corresponding HTTP
	...
	// OPERICA_LARK_SECRET_KEY is set; tests that need a no-op
	...
	// (Feishu when OPERICA_LARK_SECRET_KEY is set). The router does NOT
	...
	// + app tokens (MUL-3666). Nil unless OPERICA_SLACK_SECRET_KEY is set.
	...
	// SlackHistory backs the agent-facing `operica chat history` command: it
	...
	// "link your Operica account" prompt sent to first-time WeCom users
	...
	// OPERICA_VCS_SECRET_KEY is unset; the connect/webhook handlers return 503
...
func New(queries *db.Queries, txStarter txStarter, hub *realtime.Hub, bus *events.Bus, emailService *service.EmailService, store storage.Storage, cfSigner *auth.CloudFrontSigner, analyticsClient analytics.Client, cfg Config, daemonHubs ...*daemonws.Hub) *Handler {
	...
	// backs auto-titling. A deployment with no OPERICA_LLM_* configuration gets
