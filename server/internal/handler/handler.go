...
import (
	...
	"github.com/JTBlink/operica/server/internal/integrations/ghsnapshot"
	...
	"github.com/JTBlink/operica/server/pkg/llm"
)
...
type Config struct {
	...
	// OPERCIA_VCS_SECRET_KEY is set. It is the product boundary: the feature is
	// intended for self-hosted Opercia only (where Opercia and the Git instance
	...
	// OPERCIA_VCS_INTEGRATION_ENABLED; the self-host compose defaults it on.
	VCSIntegrationEnabled bool
	// PublicURL is the absolute base URL the API is reachable at from the
	// public internet, with no trailing slash (e.g. "https://opercia.ai").
	...
	// OPERCIA_TRUSTED_PROXIES env var (comma-separated CIDRs, e.g.
	...
	//   - LLMAPIKey       -> OPERCIA_LLM_API_KEY
	//   - LLMBaseURL       -> OPERCIA_LLM_BASE_URL (OpenAI or any compatible gateway)
	//   - LLMDefaultModel  -> OPERCIA_LLM_DEFAULT_MODEL (used when a request omits `model`)
	LLMAPIKey       string
	LLMBaseURL      string
...
type Handler struct {
	...
	// (OPERCIA_LARK_SECRET_KEY) is unset; the corresponding HTTP
	...
	// OPERCIA_LARK_SECRET_KEY is set; tests that need a no-op
	...
	// (Feishu when OPERCIA_LARK_SECRET_KEY is set). The router does NOT
	...
	// + app tokens (MUL-3666). Nil unless OPERCIA_SLACK_SECRET_KEY is set.
	...
	// SlackHistory backs the agent-facing `opercia chat history` command: it
	...
	// "link your Opercia account" prompt sent to first-time WeCom users
	...
	// OPERCIA_VCS_SECRET_KEY is unset; the connect/webhook handlers return 503
...
func New(queries *db.Queries, txStarter txStarter, hub *realtime.Hub, bus *events.Bus, emailService *service.EmailService, store storage.Storage, cfSigner *auth.CloudFrontSigner, analyticsClient analytics.Client, cfg Config, daemonHubs ...*daemonws.Hub) *Handler {
	...
	// backs auto-titling. A deployment with no OPERCIA_LLM_* configuration gets
