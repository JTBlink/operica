...
func TestGetConfigIncludesRuntimeAuthConfig(t *testing.T) {
	...
	t.Setenv("OPERICA_PUBLIC_URL", "https://api.example.com/")
	t.Setenv("OPERICA_APP_URL", "https://app.example.com/")
...
func TestGetConfigUsesAppURLForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("OPERICA_PUBLIC_URL", "")
	t.Setenv("OPERICA_APP_URL", "https://operica.internal.example/")
	...
	if cfg.DaemonServerURL != "https://operica.internal.example" {
		t.Fatalf("daemon_server_url: want same-origin URL, got %q", cfg.DaemonServerURL)
	}
	if cfg.DaemonAppURL != "https://operica.internal.example" {
...
func TestGetConfigUsesFrontendOriginForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("OPERICA_PUBLIC_URL", "")
	t.Setenv("OPERICA_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://operica.internal.example/")
...
func TestGetConfigOmitsOfficialCloudDaemonSetup(t *testing.T) {
	t.Setenv("OPERICA_PUBLIC_URL", "https://api.operica.ai")
	t.Setenv("OPERICA_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://operica.ai")
...
// frontend is operica.ai, but the deployment does not set OPERICA_PUBLIC_URL to
// the api host. Previously this fell through to the same-origin branch and
// emitted daemon_server_url=https://operica.ai, which the dialog turned into
// `operica setup self-host --server-url https://operica.ai` — pointing the
...
// setup URLs are omitted and the dialog falls back to `operica setup`.
...
// cloud frontend when it is configured through OPERICA_APP_URL.
func TestGetConfigOmitsCloudDaemonSetupForConfiguredAppURL(t *testing.T) {
	t.Setenv("OPERICA_PUBLIC_URL", "")
	t.Setenv("OPERICA_APP_URL", "https://operica.ai")
...
func TestURLHostEqualsCanonicalizesCommonHostForms(t *testing.T) {
	tests := []struct {
		...
		{name: "full URL", raw: "https://api.operica.ai", want: true},
		{name: "bare host", raw: "api.operica.ai", want: true},
		{name: "host port", raw: "api.operica.ai:8080", want: true},
		{name: "trailing dot", raw: "https://api.operica.ai.", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := urlHostEquals(tt.raw, "api.operica.ai"); got != tt.want {
...
func TestGetConfigExposesServerVersion(t *testing.T) {
	...
	t.Setenv("OPERICA_APP_URL", "https://operica.self-hosted.example")
	t.Setenv("FRONTEND_ORIGIN", "")
...
// suppressed on the managed cloud (frontend host operica.ai) even when the
...
func TestGetConfigOmitsServerVersionOnOfficialCloud(t *testing.T) {
	...
	// Official cloud: frontend host operica.ai -> version omitted.
	t.Setenv("OPERICA_APP_URL", "https://operica.ai")
	t.Setenv("FRONTEND_ORIGIN", "")
	w := httptest.NewRecorder()
	testHandler.GetConfig(w, req)
