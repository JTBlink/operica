...
func TestGetConfigIncludesRuntimeAuthConfig(t *testing.T) {
	...
	t.Setenv("OPERCIA_PUBLIC_URL", "https://api.example.com/")
	t.Setenv("OPERCIA_APP_URL", "https://app.example.com/")
...
func TestGetConfigUsesAppURLForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("OPERCIA_PUBLIC_URL", "")
	t.Setenv("OPERCIA_APP_URL", "https://opercia.internal.example/")
	...
	if cfg.DaemonServerURL != "https://opercia.internal.example" {
		t.Fatalf("daemon_server_url: want same-origin URL, got %q", cfg.DaemonServerURL)
	}
	if cfg.DaemonAppURL != "https://opercia.internal.example" {
...
func TestGetConfigUsesFrontendOriginForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("OPERCIA_PUBLIC_URL", "")
	t.Setenv("OPERCIA_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://opercia.internal.example/")
...
func TestGetConfigOmitsOfficialCloudDaemonSetup(t *testing.T) {
	t.Setenv("OPERCIA_PUBLIC_URL", "https://api.opercia.ai")
	t.Setenv("OPERCIA_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://opercia.ai")
...
// frontend is opercia.ai, but the deployment does not set OPERCIA_PUBLIC_URL to
// the api host. Previously this fell through to the same-origin branch and
// emitted daemon_server_url=https://opercia.ai, which the dialog turned into
// `opercia setup self-host --server-url https://opercia.ai` — pointing the
...
// setup URLs are omitted and the dialog falls back to `opercia setup`.
...
// cloud frontend when it is configured through OPERCIA_APP_URL.
func TestGetConfigOmitsCloudDaemonSetupForConfiguredAppURL(t *testing.T) {
	t.Setenv("OPERCIA_PUBLIC_URL", "")
	t.Setenv("OPERCIA_APP_URL", "https://opercia.ai")
...
func TestURLHostEqualsCanonicalizesCommonHostForms(t *testing.T) {
	tests := []struct {
		...
		{name: "full URL", raw: "https://api.opercia.ai", want: true},
		{name: "bare host", raw: "api.opercia.ai", want: true},
		{name: "host port", raw: "api.opercia.ai:8080", want: true},
		{name: "trailing dot", raw: "https://api.opercia.ai.", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := urlHostEquals(tt.raw, "api.opercia.ai"); got != tt.want {
...
func TestGetConfigExposesServerVersion(t *testing.T) {
	...
	t.Setenv("OPERCIA_APP_URL", "https://opercia.self-hosted.example")
	t.Setenv("FRONTEND_ORIGIN", "")
...
// suppressed on the managed cloud (frontend host opercia.ai) even when the
...
func TestGetConfigOmitsServerVersionOnOfficialCloud(t *testing.T) {
	...
	// Official cloud: frontend host opercia.ai -> version omitted.
	t.Setenv("OPERCIA_APP_URL", "https://opercia.ai")
	t.Setenv("FRONTEND_ORIGIN", "")
	w := httptest.NewRecorder()
	testHandler.GetConfig(w, req)
