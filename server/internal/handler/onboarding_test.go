...
func TestJoinCloudWaitlistRecordsEmailAndReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-ok@opercia.ai")
...
func TestJoinCloudWaitlistAllowsEmptyReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-noreason@opercia.ai")
...
func TestJoinCloudWaitlistMissingEmailReturns400(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-missing@opercia.ai")
...
func TestJoinCloudWaitlistRejectsOverlongReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-long@opercia.ai")
...
func TestJoinCloudWaitlistSecondCallOverwrites(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-overwrite@opercia.ai")
...
func TestBootstrapOnboardingRuntimeCreatesSingleGuideIssue(t *testing.T) {
	...
	if avatarURL == nil || *avatarURL != onboardingAssistantAvatarURL {
		t.Fatalf("agent avatar_url = %v, want seeded Opercia Helper avatar", avatarURL)
	}
...
func TestBootstrapOnboardingRuntime_WithStarterPrompt(t *testing.T) {
	...
	const wantPrompt = "Introduce Opercia to me, please."
	body := map[string]string{
		"workspace_id":   testWorkspaceID,
		"runtime_id":     testRuntimeID,
...
func TestBootstrapOnboardingNoRuntimeCreatesSingleGuideIssue(t *testing.T) {
	...
	for _, want := range []string{
		"Try Opercia first",
		"https://opercia.ai/docs/install-agent-runtime",
...
func TestPatchOnboardingSplitsQuestionnaireAndSourceEvents(t *testing.T) {
	userID := newWaitlistTestUser(t, "onboarding-event-split@opercia.ai")

	m := obsmetrics.NewBusinessMetrics()
	h := *testHandler
	h.Metrics = m

	const questionnaireCounter = "opercia_onboarding_questionnaire_submitted_total"
	const sourceCounter = "opercia_onboarding_source_submitted_total"
...
func TestPatchOnboardingAllSkippedResolvesBothCounters(t *testing.T) {
	userID := newWaitlistTestUser(t, "onboarding-source-skip@opercia.ai")
	...
	if got := counterValue(t, m, "opercia_onboarding_questionnaire_submitted_total"); got != 1 {
		t.Fatalf("expected questionnaire counter 1 for all-skip, got %v", got)
	}
	if got := counterValue(t, m, "opercia_onboarding_source_submitted_total"); got != 1 {
