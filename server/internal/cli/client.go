...
// Set by the operica binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the Operica server API.
...
// reading a capped slice of the body. Every Operica API helper funnels its
...
// It can be overridden with the OPERICA_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring OPERICA_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("OPERICA_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors OPERICA_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects OPERICA_HTTP_TIMEOUT.
...
// CLI never hands an attachment URL to a native loader — `operica attachment
