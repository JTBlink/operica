...
// Set by the opercia binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the Opercia server API.
...
// reading a capped slice of the body. Every Opercia API helper funnels its
...
// It can be overridden with the OPERCIA_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring OPERCIA_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("OPERCIA_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors OPERCIA_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects OPERCIA_HTTP_TIMEOUT.
...
// CLI never hands an attachment URL to a native loader — `opercia attachment
