...
const (
	defaultCLIConfigPath = ".operica/config.json"

	// TaskConfigRootEnv points daemon-managed CLI invocations at a private,
	// per-task Operica config directory. It is deliberately Operica-specific:
	...
	TaskConfigRootEnv = "OPERICA_TASK_CONFIG_ROOT"
)
...
type CLIConfig struct {
	...
	// OPERICA_DAEMON_DEVICE_NAME env, this field, os.Hostname().
	...
	// OPERICA_AGENT_RUNTIME_NAME env, this field, the built-in
	// DefaultRuntimeName.
	...
	// OPERICA_DAEMON_MAX_CONCURRENT_TASKS env, this field, default.
	...
	// OPERICA_DAEMON_POLL_INTERVAL env, this field, DefaultPollInterval.
	...
	// flag, OPERICA_DAEMON_HEARTBEAT_INTERVAL env, this field,
	...
	// explicit 0), OPERICA_AGENT_TIMEOUT env, this field,
	// DefaultAgentTimeout.
	...
	// OPERICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT env, this field,
	...
	// OPERICA_CODEX_HANDSHAKE_TIMEOUT env, this field,
	...
	// already resolves to enabled on Operica Cloud. Absent / false means
	// "let env/default decide". Resolution precedence:
	// --no-auto-update flag, OPERICA_DAEMON_AUTO_UPDATE=false env, this
	...
	// --auto-update-interval flag, OPERICA_DAEMON_AUTO_UPDATE_INTERVAL
	...
	// operica binary that was replaced on disk out of band. Single-direction
	...
	// --no-auto-reload flag, OPERICA_DAEMON_AUTO_RELOAD=false env, this
	...
	// `operica runtime profile set-path`; the daemon prefers it over the
...
//
//	BinaryPath: OPERICA_OPENCLAW_PATH (env)  > backends.openclaw.binary_path > PATH lookup
...
// OPERICA_OPENCLAW_STATE_DIR. Rationale: OpenClaw already honors its own env
...
// OpenClaw's own env, OPERICA_OPENCLAW_STATE_DIR can be layered on top
...
// An empty profile returns the default path (~/.operica/config.json).
// A named profile returns ~/.operica/profiles/<name>/config.json.
...
func CLIConfigPathForProfile(profile string) (string, error) {
	root, taskLocal, err := opericaConfigRoot()
	if err != nil {
	...
	return filepath.Join(root, ".operica", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.operica/. A named profile returns ~/.operica/profiles/<name>/.
...
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		...
		return filepath.Join(root, ".operica"), nil
	}
	if taskLocal {
		return filepath.Join(root, "profiles", profile), nil
	}
	return filepath.Join(root, ".operica", "profiles", profile), nil
}

func opericaConfigRoot() (root string, taskLocal bool, err error) {
...
func validateTaskLocalProfile(profile string) error {
	...
	if profile == "." || profile == ".." || filepath.IsAbs(profile) || strings.ContainsAny(profile, `/\\`) || filepath.Clean(profile) != profile {
		return fmt.Errorf("invalid task-local Operica profile name %q", profile)
	}
	return nil
}
