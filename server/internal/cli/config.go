...
const (
	defaultCLIConfigPath = ".opercia/config.json"

	// TaskConfigRootEnv points daemon-managed CLI invocations at a private,
	// per-task Opercia config directory. It is deliberately Opercia-specific:
	...
	TaskConfigRootEnv = "OPERCIA_TASK_CONFIG_ROOT"
)
...
type CLIConfig struct {
	...
	// OPERCIA_DAEMON_DEVICE_NAME env, this field, os.Hostname().
	...
	// OPERCIA_AGENT_RUNTIME_NAME env, this field, the built-in
	// DefaultRuntimeName.
	...
	// OPERCIA_DAEMON_MAX_CONCURRENT_TASKS env, this field, default.
	...
	// OPERCIA_DAEMON_POLL_INTERVAL env, this field, DefaultPollInterval.
	...
	// flag, OPERCIA_DAEMON_HEARTBEAT_INTERVAL env, this field,
	...
	// explicit 0), OPERCIA_AGENT_TIMEOUT env, this field,
	// DefaultAgentTimeout.
	...
	// OPERCIA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT env, this field,
	...
	// OPERCIA_CODEX_HANDSHAKE_TIMEOUT env, this field,
	...
	// already resolves to enabled on Opercia Cloud. Absent / false means
	// "let env/default decide". Resolution precedence:
	// --no-auto-update flag, OPERCIA_DAEMON_AUTO_UPDATE=false env, this
	...
	// --auto-update-interval flag, OPERCIA_DAEMON_AUTO_UPDATE_INTERVAL
	...
	// opercia binary that was replaced on disk out of band. Single-direction
	...
	// --no-auto-reload flag, OPERCIA_DAEMON_AUTO_RELOAD=false env, this
	...
	// `opercia runtime profile set-path`; the daemon prefers it over the
...
//
//	BinaryPath: OPERCIA_OPENCLAW_PATH (env)  > backends.openclaw.binary_path > PATH lookup
...
// OPERCIA_OPENCLAW_STATE_DIR. Rationale: OpenClaw already honors its own env
...
// OpenClaw's own env, OPERCIA_OPENCLAW_STATE_DIR can be layered on top
...
// An empty profile returns the default path (~/.opercia/config.json).
// A named profile returns ~/.opercia/profiles/<name>/config.json.
...
func CLIConfigPathForProfile(profile string) (string, error) {
	root, taskLocal, err := operciaConfigRoot()
	if err != nil {
	...
	return filepath.Join(root, ".opercia", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.opercia/. A named profile returns ~/.opercia/profiles/<name>/.
...
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		...
		return filepath.Join(root, ".opercia"), nil
	}
	if taskLocal {
		return filepath.Join(root, "profiles", profile), nil
	}
	return filepath.Join(root, ".opercia", "profiles", profile), nil
}

func operciaConfigRoot() (root string, taskLocal bool, err error) {
...
func validateTaskLocalProfile(profile string) error {
	...
	if profile == "." || profile == ".." || filepath.IsAbs(profile) || strings.ContainsAny(profile, `/\\`) || filepath.Clean(profile) != profile {
		return fmt.Errorf("invalid task-local Opercia profile name %q", profile)
	}
	return nil
}
