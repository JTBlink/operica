...
func TestClassifyNetworkError(t *testing.T) {
	cases := []struct {
		...
		{"dns", &net.DNSError{Err: "no such host", Name: "api.operica.ai", IsNotFound: true}, KindNetworkDNS},
		...
		{"x509 hostname", x509.HostnameError{Host: "api.operica.ai"}, KindNetworkTLS},
		{"timeout string fallback", errors.New("Get \"https://x\": net/http: request canceled (Client.Timeout exceeded)"), KindNetworkTimeout},
		{"dns string fallback", errors.New("dial tcp: lookup api.operica.ai: no such host"), KindNetworkDNS},
...
func TestFormatErrorNetwork(t *testing.T) {
	...
	raw := errors.New("Get \"https://api.operica.ai/api/issues/abc\": context deadline exceeded")
	...
	if strings.Contains(got, "api.operica.ai") || strings.Contains(got, "resolve issue") {
...
func TestHTTPTimeout(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("OPERICA_HTTP_TIMEOUT", tc.val)
...
func TestFormatErrorActionableHints(t *testing.T) {
	cases := []struct {
		...
		{401, []string{"operica login", "self-hosted", "administrator"}, []string{"operica login", "自托管", "管理员"}},
...
// `operica login` failure guidance visible without losing classification.
func TestUserMessageError(t *testing.T) {
	withLang(t, "en_US.UTF-8")
	const hint = "Could not sign in with that token — make sure it is valid and not expired, then run `operica login --token <token>` again."
	...
	t.Run("wrapped NetworkError classifies as network", func(t *testing.T) {
		...
		err := WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `operica login` again.", underlying)
