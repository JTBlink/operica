...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	OperciaUserID  pgtype.UUID // the installer's Opercia account
...
// identity is the supplied operciaUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Opercia user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, operciaUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	isMember, err := qtx.IsWorkspaceMember(ctx, row.WorkspaceID, operciaUserID)
	if err != nil {
	...
	if err != nil {
		...
		// opercia_user_id differs from ours, so the WHERE clause on
...
// (installation_id, lark_open_id) when opercia_user_id matches (the
...
func (s *BindingTokenService) BindInstallerTx(ctx context.Context, qtx *ChannelStore, p InstallerBindParams) error {
	...
	isMember, err := q.IsWorkspaceMember(ctx, p.WorkspaceID, p.OperciaUserID)
	if err != nil {
...
// open_id) pair and points at a different Opercia user. Account
