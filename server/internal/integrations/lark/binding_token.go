...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	OpericaUserID  pgtype.UUID // the installer's Operica account
...
// identity is the supplied opericaUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Operica user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, opericaUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	isMember, err := qtx.IsWorkspaceMember(ctx, row.WorkspaceID, opericaUserID)
	if err != nil {
	...
	if err != nil {
		...
		// operica_user_id differs from ours, so the WHERE clause on
...
// (installation_id, lark_open_id) when operica_user_id matches (the
...
func (s *BindingTokenService) BindInstallerTx(ctx context.Context, qtx *ChannelStore, p InstallerBindParams) error {
	...
	isMember, err := q.IsWorkspaceMember(ctx, p.WorkspaceID, p.OpericaUserID)
	if err != nil {
...
// open_id) pair and points at a different Operica user. Account
