...
VALUES ($1, $2, 'wecom bot name runtime', 'local', 'opercia_daemon') ON CONFLICT (id) DO NOTHING`, wcNameRuntime, wcNameWS)
...
func TestRotatingTheSecretKeepsTheBotName(t *testing.T) {
	...
	if _, err := svc.Upsert(ctx, botNameParams(wcNameBotA, "Opercia Bot")); err != nil {
	...
	if again.BotDisplayName != "Opercia Bot" {
...
func TestTheBotNameSurvivesTheConfigRoundTrip(t *testing.T) {
	t.Parallel()
	cfg, err := encodeInstallConfig(Installation{BotID: "bot-xyz", BotDisplayName: "Opercia Bot"})
	if err != nil {
	...
	if out.BotDisplayName != "Opercia Bot" {
		t.Fatalf("BotDisplayName round trip = %q, want Opercia Bot", out.BotDisplayName)
	}
}
