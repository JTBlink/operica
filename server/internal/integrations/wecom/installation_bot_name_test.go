...
VALUES ($1, $2, 'wecom bot name runtime', 'local', 'operica_daemon') ON CONFLICT (id) DO NOTHING`, wcNameRuntime, wcNameWS)
...
func TestRotatingTheSecretKeepsTheBotName(t *testing.T) {
	...
	if _, err := svc.Upsert(ctx, botNameParams(wcNameBotA, "Operica Bot")); err != nil {
	...
	if again.BotDisplayName != "Operica Bot" {
...
func TestTheBotNameSurvivesTheConfigRoundTrip(t *testing.T) {
	t.Parallel()
	cfg, err := encodeInstallConfig(Installation{BotID: "bot-xyz", BotDisplayName: "Operica Bot"})
	if err != nil {
	...
	if out.BotDisplayName != "Operica Bot" {
		t.Fatalf("BotDisplayName round trip = %q, want Operica Bot", out.BotDisplayName)
	}
}
