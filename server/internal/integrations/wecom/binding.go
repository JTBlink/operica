...
// page, and their WeCom userid is bound to their Opercia account. Mirrors
...
var (
	...
	// different Opercia user (account transfer must go through explicit
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, operciaUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	if _, err := qtx.CreateChannelUserBinding(ctx, db.CreateChannelUserBindingParams{
		...
		// user — the ON CONFLICT DO UPDATE WHERE opercia_user_id=… gating
		// rejected it.
		if errors.Is(err, pgx.ErrNoRows) {
