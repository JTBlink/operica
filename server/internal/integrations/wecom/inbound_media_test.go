...
// mention arrives glued to the front of the text run: "@Operica Bot /issue
...
// parser sees "@Operica Bot", drop the placeholder removal and it sees
...
func TestDispatchFrame_GroupMentionedMixedCommand(t *testing.T) {
	...
	env := mixedFrame(t, group,
		...
		textRun("@Operica Bot /issue 登录坏了"),
	)
	got, called, _ := dispatchOneAs(t, env, "Operica Bot")
	if !called {
	...
	if want := "[Image]\n@Operica Bot /issue 登录坏了"; got.Text != want {
...
// "@Operica Bot /issue 登录坏了". WeCom delivers one 图文混排 with an image run
...
func TestDispatchFrame_GroupMentionedSpokenIssueWithAnImage(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			items:    []map[string]any{imageRun("https://cos.example.com/a"), voiceRun("@Operica Bot /issue 登录坏了")},
			wantBody: "[Image]\n@Operica Bot /issue 登录坏了",
		},
		{
			...
			items:    []map[string]any{textRun("@Operica Bot"), imageRun("https://cos.example.com/a"), voiceRun("/issue 登录坏了")},
			wantBody: "@Operica Bot\n[Image]\n/issue 登录坏了",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got, called, _ := dispatchOneAs(t, mixedFrame(t, group, tc.items...), "Operica Bot")
			if !called {
				t.Fatal("the group message never reached the handler")
			}
			if got.Source.ChatType != channel.ChatTypeGroup {
