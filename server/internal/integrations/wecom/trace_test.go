...
// trace_test.go — guards for the OPERCIA_WECOM_TRACE operator switch. Six
...
// with OPERCIA_WECOM_TRACE unset, a real inbound message and the frames around
...
// and puts it in a URL inside the message body; with a normal OPERCIA_APP_URL
...
func TestTraceNeverLogsABindingToken(t *testing.T) {
	...
	r := NewOutboundReplier(OutboundReplierConfig{Senders: reg, AppURL: "https://opercia.example"})
...
func TestBindingPromptFitsInsideThePreviewCap(t *testing.T) {
	...
	bindURL := "https://opercia.example" + "/wecom/bind" + "?token=" + url.QueryEscape(rawToken)
	upToTokenEnd := "👋 请先绑定你的 Opercia 账号，才能与我对话：\n" + bindURL
...
// cut lands inside an escape — so an operator who turned OPERCIA_WECOM_TRACE
