...
// trace_test.go — guards for the OPERICA_WECOM_TRACE operator switch. Six
...
// with OPERICA_WECOM_TRACE unset, a real inbound message and the frames around
...
// and puts it in a URL inside the message body; with a normal OPERICA_APP_URL
...
func TestTraceNeverLogsABindingToken(t *testing.T) {
	...
	r := NewOutboundReplier(OutboundReplierConfig{Senders: reg, AppURL: "https://operica.example"})
...
func TestBindingPromptFitsInsideThePreviewCap(t *testing.T) {
	...
	bindURL := "https://operica.example" + "/wecom/bind" + "?token=" + url.QueryEscape(rawToken)
	upToTokenEnd := "👋 请先绑定你的 Operica 账号，才能与我对话：\n" + bindURL
...
// cut lands inside an escape — so an operator who turned OPERICA_WECOM_TRACE
