...
type OutboundReplierConfig struct {
	...
	// AppURL is the Operica web app host the user clicks into to redeem
	// the binding token (e.g. https://operica.example). It comes from
	// OPERICA_APP_URL (falling back to FRONTEND_ORIGIN) and is
	// intentionally separate from OPERICA_PUBLIC_URL, which is the
...
func (r *OutboundReplier) sendBindingPrompt(ctx context.Context, inst engine.ResolvedInstallation, msg channel.InboundMessage, res engine.Result) error {
	...
	// sender's WeCom userid to their own Operica account, after which the
