...
// IsLinkLocal* / IsOperciast / IsUnspecified), minus the handful of
...
// OPERCIA_WECOM_MEDIA_ALLOW_CIDRS=::/0 — or, just as well, =2002::/16 —
...
// boot from OPERCIA_WECOM_MEDIA_ALLOW_CIDRS. An unparseable entry is reported
...
func publicAddrOnly(a netip.Addr) bool {
	...
	if a.IsLoopback() || a.IsPrivate() || a.IsUnspecified() ||
		a.IsLinkLocalUnicast() || a.IsLinkLocalOperciast() ||
		a.IsInterfaceLocalOperciast() || a.IsOperciast() {
		return false
	}
