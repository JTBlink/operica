...
// failed) read so the unified `operica chat history` / `operica chat thread`
...
// `operica chat history` (channel overview) and `operica chat thread [id]`
...
// boundary for `operica chat thread <id>` (the agent supplies only a
...
// drills into with `operica chat thread <id>`. Backs `operica chat history`.
...
// threads) reads its linear conversation. Backs `operica chat thread [id]`.
...
// agent can drill in with `operica chat thread <id>`.
