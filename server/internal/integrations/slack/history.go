...
// failed) read so the unified `opercia chat history` / `opercia chat thread`
...
// `opercia chat history` (channel overview) and `opercia chat thread [id]`
...
// boundary for `opercia chat thread <id>` (the agent supplies only a
...
// drills into with `opercia chat thread <id>`. Backs `opercia chat history`.
...
// threads) reads its linear conversation. Backs `opercia chat thread [id]`.
...
// agent can drill in with `opercia chat thread <id>`.
