...
require_config "$config" 'OPERCIA_APP_URL: http://localhost:3100'
...
local_env="$(
  env -i PATH="$PATH" bash -c '
    ...
    printf "%s\n" \
      ...
      "OPERCIA_APP_URL=${OPERCIA_APP_URL}" \
      "GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}" \
      "OPERCIA_SERVER_URL=${OPERCIA_SERVER_URL}" \
...
require_env "$local_env" 'OPERCIA_APP_URL=http://localhost:3100'
require_env "$local_env" 'GOOGLE_REDIRECT_URI=http://localhost:3100/auth/callback'
require_env "$local_env" 'OPERCIA_SERVER_URL=ws://localhost:9100/ws'
...
require_env "$(cat "$worktree_env")" "OPERCIA_PUBLIC_URL=http://localhost:${worktree_backend_port}"
...
grep -v '^OPERCIA_PUBLIC_URL=' "$worktree_env" >"$old_worktree_env"
require_env \
...
printf '\nOPERCIA_PUBLIC_URL=https://api.explicit.example\n' >>"$explicit_worktree_env"
require_env \
