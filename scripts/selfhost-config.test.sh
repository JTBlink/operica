...
require_config "$config" 'OPERICA_APP_URL: http://localhost:3100'
...
local_env="$(
  env -i PATH="$PATH" bash -c '
    ...
    printf "%s\n" \
      ...
      "OPERICA_APP_URL=${OPERICA_APP_URL}" \
      "GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}" \
      "OPERICA_SERVER_URL=${OPERICA_SERVER_URL}" \
...
require_env "$local_env" 'OPERICA_APP_URL=http://localhost:3100'
require_env "$local_env" 'GOOGLE_REDIRECT_URI=http://localhost:3100/auth/callback'
require_env "$local_env" 'OPERICA_SERVER_URL=ws://localhost:9100/ws'
...
require_env "$(cat "$worktree_env")" "OPERICA_PUBLIC_URL=http://localhost:${worktree_backend_port}"
...
grep -v '^OPERICA_PUBLIC_URL=' "$worktree_env" >"$old_worktree_env"
require_env \
...
printf '\nOPERICA_PUBLIC_URL=https://api.explicit.example\n' >>"$explicit_worktree_env"
require_env \
