...
| `operica-tools.sh start [--server] [--web] [--all]` | `make run` | 启动前先停止当前 checkout 的已有开发进程，再启动 Electron 桌面端 (`pnpm dev:desktop`)；本地后端未运行时会自动附加。`--server` 会显式确保后端并加载 env、确保 Postgres、编译本机二进制、执行迁移；`--web` 会额外启动 Web；`--all` 同时附加后端和 Web。 |
