...
echo ""
echo "✓ Operica is running!"
echo "  Frontend: ${frontend_url}"
echo "  Backend:  ${backend_url}"
echo ""
if [ "$mode" = "build" ]; then
  echo "Built images locally via docker-compose.selfhost.build.yml."
  echo "Local tags: operica-backend:dev and operica-web:dev."
else
  echo "Images: ${OPERICA_BACKEND_IMAGE:-ghcr.io/JTBlink/operica-backend}:${OPERICA_IMAGE_TAG:-latest}"
  echo "        ${OPERICA_WEB_IMAGE:-ghcr.io/JTBlink/operica-web}:${OPERICA_IMAGE_TAG:-latest}"
fi
echo ""
...
echo "  brew install operica-ai/tap/operica"
echo "  operica setup self-host"
