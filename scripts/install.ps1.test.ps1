...
$runnerScript = Join-Path ([System.IO.Path]::GetTempPath()) "opercia-install-ps1-case.ps1"
...
if (-not $env:USERPROFILE) { $env:USERPROFILE = [System.IO.Path]::GetTempPath() }
$env:OPERCIA_INSTALL_DIR = $WorkDir
$env:OPERCIA_SELFHOST_REF = "main"
...
foreach ($case in $cases) {
    $workDir = Join-Path ([System.IO.Path]::GetTempPath()) ("opercia-ps1-" + [guid]::NewGuid().ToString("N"))
