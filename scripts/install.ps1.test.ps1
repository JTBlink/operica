...
$runnerScript = Join-Path ([System.IO.Path]::GetTempPath()) "operica-install-ps1-case.ps1"
...
if (-not $env:USERPROFILE) { $env:USERPROFILE = [System.IO.Path]::GetTempPath() }
$env:OPERICA_INSTALL_DIR = $WorkDir
$env:OPERICA_SELFHOST_REF = "main"
...
foreach ($case in $cases) {
    $workDir = Join-Path ([System.IO.Path]::GetTempPath()) ("operica-ps1-" + [guid]::NewGuid().ToString("N"))
