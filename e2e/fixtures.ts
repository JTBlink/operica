...
const DATABASE_URL = process.env.DATABASE_URL ?? "postgres://operica:operica@localhost:5432/operica?sslmode=disable";
...
export class TestApiClient {
  ...
  async login(email: string, name: string) {
    ...
    try {
      ...
      const configuredDevCode = process.env.OPERICA_DEV_VERIFICATION_CODE?.trim();
