...
const DATABASE_URL = process.env.DATABASE_URL ?? "postgres://opercia:opercia@localhost:5432/opercia?sslmode=disable";
...
export class TestApiClient {
  ...
  async login(email: string, name: string) {
    ...
    try {
      ...
      const configuredDevCode = process.env.OPERCIA_DEV_VERIFICATION_CODE?.trim();
