// Brand host shown as the workspace URL prefix on the managed Operica Cloud,
...
// self-hosted server that has not set OPERICA_APP_URL / FRONTEND_ORIGIN), so
// this literal must remain the ultimate fallback.
const BRAND_WORKSPACE_HOST = "operica.ai";
 ...
 * self-hosted instances show their own domain instead of `operica.ai`. Falls
