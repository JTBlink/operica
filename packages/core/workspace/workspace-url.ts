// Brand host shown as the workspace URL prefix on the managed Opercia Cloud,
...
// self-hosted server that has not set OPERCIA_APP_URL / FRONTEND_ORIGIN), so
// this literal must remain the ultimate fallback.
const BRAND_WORKSPACE_HOST = "opercia.ai";
 ...
 * self-hosted instances show their own domain instead of `opercia.ai`. Falls
