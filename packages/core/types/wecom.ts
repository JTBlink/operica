 ...
 * Opercia agent. Wire shape mirrors `WecomInstallationResponse` in
...
export interface ListWecomInstallationsResponse {
  ...
  /** Whether OPERCIA_WECOM_SECRET_KEY is set on this deployment. When false the
 ...
 * with the deployment's OPERCIA_WECOM_SECRET_KEY before writing it, so
...
export interface RegisterWecomBYORequest {
   ...
   * containing a space ("Opercia Bot") otherwise swallows the slash command
 ...
 * bound to the logged-in Opercia user in this workspace/installation. */
