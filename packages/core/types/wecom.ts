 ...
 * Operica agent. Wire shape mirrors `WecomInstallationResponse` in
...
export interface ListWecomInstallationsResponse {
  ...
  /** Whether OPERICA_WECOM_SECRET_KEY is set on this deployment. When false the
 ...
 * with the deployment's OPERICA_WECOM_SECRET_KEY before writing it, so
...
export interface RegisterWecomBYORequest {
   ...
   * containing a space ("Operica Bot") otherwise swallows the slash command
 ...
 * bound to the logged-in Operica user in this workspace/installation. */
