...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to Opercia' title", () => {
    renderWithI18n(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/sign in to opercia/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("opercia_token", "existing-jwt");
