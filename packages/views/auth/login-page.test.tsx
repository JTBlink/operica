...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to Operica' title", () => {
    renderWithI18n(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/sign in to operica/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("operica_token", "existing-jwt");
