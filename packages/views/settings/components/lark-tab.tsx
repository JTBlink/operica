...
import { larkInstallationsOptions, larkKeys } from "@operica/core/lark";
import { api, ApiError } from "@operica/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@operica/core/types";
...
// install path is per-agent (each Operica Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Operica Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Operica agent's
