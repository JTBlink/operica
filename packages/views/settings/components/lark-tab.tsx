...
import { larkInstallationsOptions, larkKeys } from "@opercia/core/lark";
import { api, ApiError } from "@opercia/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@opercia/core/types";
...
// install path is per-agent (each Opercia Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Opercia Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Opercia agent's
