...
vi.mock("@operica/ui/components/ui/collapsible", () => ({
...
vi.mock("@operica/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));
...
vi.mock("@operica/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@operica/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@operica/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@operica/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@operica/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@operica/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@operica/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@operica/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@operica/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
