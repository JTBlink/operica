...
vi.mock("@opercia/ui/components/ui/sidebar", () => ({
...
vi.mock("@opercia/ui/components/ui/collapsible", () => ({
...
vi.mock("@opercia/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));
...
vi.mock("@opercia/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@opercia/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@opercia/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@opercia/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@opercia/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@opercia/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@opercia/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@opercia/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@opercia/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
