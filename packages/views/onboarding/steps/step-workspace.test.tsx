...
// self-hosted instances instead of the hardcoded `operica.ai`.
describe("StepWorkspace — workspace URL prefix", () => {
  it("shows the brand host when no app URL is configured", () => {
    renderStep({ existing: null, disabled: false });
    expect(screen.getByText("operica.ai/")).toBeInTheDocument();
  });
  ...
  it("shows the deployment host for self-hosted instances", () => {
    renderStep({
      existing: null,
      disabled: false,
      daemonAppUrl: "https://operica.example.com",
    });
    expect(screen.getByText("operica.example.com/")).toBeInTheDocument();
    expect(screen.queryByText("operica.ai/")).not.toBeInTheDocument();
  });
});
