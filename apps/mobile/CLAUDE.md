...
- `import type` from `@opercia/core/types/*` (zero runtime coupling)
- Pure functions from `@opercia/core/`
...
- **Main CI** (`.github/workflows/ci.yml`) excludes mobile via `--filter='!@opercia/mobile'`. Mobile failures do NOT block web/desktop PRs.
...
1. **Read the payload.** Find the event in `@opercia/core/types/events.ts`. Note the fields; decide if patch is possible (full object) or invalidate is required (just an id).
