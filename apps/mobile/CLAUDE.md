...
- `import type` from `@operica/core/types/*` (zero runtime coupling)
- Pure functions from `@operica/core/`
...
- **Main CI** (`.github/workflows/ci.yml`) excludes mobile via `--filter='!@operica/mobile'`. Mobile failures do NOT block web/desktop PRs.
...
1. **Read the payload.** Find the event in `@operica/core/types/events.ts`. Note the fields; decide if patch is possible (full object) or invalidate is required (just an id).
