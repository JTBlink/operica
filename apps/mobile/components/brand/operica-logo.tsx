/**
 * Operica wordmark / sigil. Keep this vector and the product SVG in sync.
 *
 * react-native-svg does not resolve CSS `currentColor`, so callers must pass
 * `color` explicitly. For theme-aware usage, pair with `useColorScheme` and
 * the `THEME` token from `@/lib/theme`.
 */
...
interface OpericaLogoProps {
  size?: number;
  color?: string;
}

export function OpericaLogo({ size = 48, color }: OpericaLogoProps) {
