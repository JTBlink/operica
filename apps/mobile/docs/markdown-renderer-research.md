...
**Scope:** `apps/mobile/` — choosing a markdown renderer for Opercia iOS that
...
| **`expo-image`** | **Pick for inline** | First-party Expo, on-disk cache, `contentFit` API, `transition` prop. Same engine the rest of the Opercia stack will use for avatars later |
...
│   ├── renderer.tsx              # OperciaRenderer extends Renderer — overrides link, image
...
class OperciaRenderer extends Renderer {
...
Mobile must convert legacy → new before parsing, otherwise old comments render the literal shortcode text. Logic is a pure regex transform — `packages/ui/markdown/index.ts` exports `preprocessMentionShortcodes` for web/desktop, but mobile **cannot** import from `@opercia/ui/*` (Sharing Principles in `apps/mobile/CLAUDE.md`).

Two options:

| Option | Trade-off |
|---|---|
| **A.** Lift the pure function to `@opercia/core/markdown/` so all three apps share it | One PR adds a new core module; web/desktop migrate their import path; mobile imports same function. Single source of truth |
