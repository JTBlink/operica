---
title: Using the CLI
description: Install and log in to the Operica CLI, and manage workspaces, issues, agents, and runtimes from the terminal.
---

import { Callout } from "fumadocs-ui/components/callout";

The Operica CLI both connects local runtimes and manages workspaces, issues, agents, and automations from the terminal. This page covers the common paths; the flags your installed version supports are always defined by each command's own `--help`.
...
If you only need to sign in again without overwriting other configuration, run `operica login`. On a machine without a browser, create a personal access token in the web settings first, then enter it with:
...
Subsequent commands use this workspace. A single command can override it with `--workspace-id`, or you can set `OPERICA_WORKSPACE_ID`.

Invite members:

```bash
operica workspace member invite teammate@example.com
operica workspace member invite admin@example.com --role admin
```
...
# View and search
operica issue list
operica issue get MUL-123
operica issue search "login failure"

# Create and update
operica issue create --title "Fix login failure"
operica issue status MUL-123 in_progress
operica issue assign MUL-123 --to "Backend Agent"

# Comments and runs
operica issue comment list MUL-123
operica issue comment add MUL-123 --content "Check the regression tests first"
operica issue runs MUL-123
```

Read long descriptions or comments from stdin to avoid dealing with newlines and quoting:

```bash
operica issue create --title "Upgrade notes" --description-stdin < notes.md
operica issue comment add MUL-123 --content-stdin < review.md
```

View the messages of a single run, or stop a task:

```bash
operica issue run-messages <task-id> --issue MUL-123
operica issue cancel-task <task-id> --issue MUL-123
```
...
```bash
operica agent list
operica agent get <agent-id>
operica agent create --help
operica agent update <agent-id> --help

operica skill list
operica skill get <skill-id>
operica skill import --url <skill-url>
operica agent skills add <agent-id> --skill-ids <skill-id>
```

When an import hits a skill with the same name, the default is to stop without modifying existing content. Pick by intent:

```bash
operica skill import --url <skill-url> --on-conflict overwrite
operica skill import --url <skill-url> --on-conflict rename
operica skill import --url <skill-url> --on-conflict skip
```
...
operica daemon logs --follow

operica runtime list
operica runtime rename <runtime-id> "Office Mac"
operica runtime usage <runtime-id>
operica runtime activity <runtime-id>
```

Deleting a runtime that still has active agents bound to it is refused by default. `operica runtime delete <id> --cascade` unbinds those agents, preserves their configuration and history, and cancels their active tasks.
...
`operica chat` reads **the external chat session an agent is currently handling**; it is mainly for agents in chat integrations, not a general command for browsing arbitrary workspace conversations.
...
Scripts should use JSON output rather than parsing the terminal-oriented tables. The output formats and pagination flags each command supports are defined by `operica <command> --help`.

## Profiles and configuration

The default configuration lives in `~/.operica/config.json`. Use `--profile <name>` to isolate a separate set of server address, token, default workspace, and daemon state:

```bash
operica setup self-host --profile staging \
  --server-url https://api.staging.example.com \
  --app-url https://app.staging.example.com

operica issue list --profile staging
```

Named profiles live in `~/.operica/profiles/<name>/config.json`. Inspect the current values:

```bash
operica config show
operica config show --profile staging
```

When the CLI runs inside a daemon-managed agent task, it does not load or modify these human-owned profile files. Operica API commands authenticate with the task-scoped credential injected by the daemon. `config show` and `config set` use private task-local state; human/local commands such as `login`, `logout`, `setup`, `workspace switch`, local runtime profile path changes, `daemon start` / `stop` / `restart`, `daemon logs`, and `daemon probe-runtimes` are unavailable. `auth status` does not print token material in this context.
...
| Command | Subcommand | Purpose | Key flags |
| --- | --- | --- | --- |
| `daemon` | `start` | Start the local daemon | `--foreground`, `--device-name`, `--runtime-name`, `--poll-interval`, `--heartbeat-interval`, `--agent-timeout` (`0` means no limit), `--max-concurrent-tasks`, `--no-auto-update`; each has a matching `OPERICA_*` environment variable |
