...
First work out which layer the problem lives in: the Operica service, the daemon, the runtime, or the AI coding tool. These commands are usually enough to surface the first meaningful error:

```bash
operica version
operica auth status
operica daemon status --output json
operica daemon logs --lines 100
```
...
- The daemon is connected to the wrong Operica service.
...
Self-hosted deployments should also request the API's `/health` from the **executing computer** — testing on the server itself cannot expose DNS, TLS, or firewall problems on the executing computer's side. To change the address, rerun `operica setup self-host`, or check the current profile's `server_url`:
...
If the runtime list is missing an expected tool, first confirm the tool runs and is signed in under the same system account and `PATH`, then run `operica daemon restart`.

### `waiting_local_directory` status

This means another in-flight task is using the same local directory. Operica waits for the directory lock to be released so two agents never modify the same files at once.

Usually you just wait for the earlier task to finish. If it is stuck, stop it from its execution log, or pick a different local directory for the current agent. This directory mutex lives in the daemon's memory — no lock file is written to disk. If you suspect stale lock state, `operica daemon restart` releases it; there is nothing to delete manually.
...
When `SMTP_HOST` and Resend are both configured, Operica prefers SMTP. For configuration, see [Sign-in and signup](/auth-setup#email-verification-codes).
...
If it is another Operica checkout, run `make stop` in that directory first. Otherwise stop the offending program normally, or change the current service's port. Public ports 80/443 are listened on by a reverse proxy such as Caddy or Nginx.

## Log locations

| Component | How to view |
| --- | --- |
| Background daemon | `operica daemon logs --lines 100` |
| Follow daemon logs live | `operica daemon logs --follow` |
| Default profile's log file | `~/.operica/daemon.log` |
| Default profile's startup or crash log | `~/.operica/daemon.err.log` |
| Named profiles | the matching logs under `~/.operica/profiles/<name>/` |
