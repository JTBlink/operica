---
title: プロジェクトアーキテクチャ
description: Operica のクライアント、Go サービス、実行デーモン、共有フロントエンドパッケージがどのように連携するかを説明します。
---

Operica は Go バックエンド、複数のクライアント、実行用コンピューター上で動作するデーモンで構成されています。PostgreSQL がコラボレーションデータを保存し、デーモンがタスクを取得してローカルの AI コーディングツールを呼び出します。
...
プロダクト利用者向けの説明は [Operica の仕組み](/how-operica-works)を参照してください。このページではコードの階層構造に焦点を当てます。
...
Mobile はこれらの React ページを再利用しません。`@operica/core` の型や純粋関数をインポートできますが、UI、query key、状態、リアルタイム購読、リリースフローは独自に持ちます。
...
Operica には異なる 2 つの WebSocket パスがあります。
