...
If you write Operica code, change a translation, or write Chinese product copy, this is the page to reference.
...
| `packages/ui` | nothing | `@operica/core`, business logic |
...
| `apps/mobile/` | types and pure functions from `@operica/core` | Web/Desktop React pages, stores, and platform implementations |
...
### The core distinction: everyday noun vs Operica-specific term

Operica's product nouns split into two categories:
...
- **Operica-specific term** — a concept no local word carries (`skill`), or a schema-level identifier a user may have to type or match (`todo`, `in_progress`, `task_id`). Render as **lowercase English** so it reads like a type name.
...
- **Code references and literal commands** stay English: `operica issue ...`, the `/issue` Slack and Lark slash command.
- **`skill`** keeps lowercase English in Chinese text — a Operica-specific concept with no established Chinese term; titles may capitalize as `Skills`.
...
**Why `issue` is translated while `skill` is not**: users file and read issues all day, and "issue" has no meaning in Chinese, Japanese, or Korean outside of dev jargon. The everyday task word is what people already say for this. `skill` is a Operica-specific concept where no local word carries the meaning.
...
| Category | Terms |
| --- | --- |
| Brands | **Operica**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
