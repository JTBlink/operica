---
title: Desktop app
description: Install Opercia Desktop, use desktop tabs and the built-in daemon, and connect to a self-hosted instance.
---

import { Callout } from "fumadocs-ui/components/callout";

Opercia Desktop is available for macOS, Windows, and Linux. It uses the same account and workspace data as the web app, but it manages the daemon on your machine automatically and keeps a separate set of tabs for each workspace.
...
You can be signed in to both at the same time. As long as they connect to the same Opercia service, the data they show is shared.

## Installation

Pick the installer that matches your OS and CPU architecture from the [Opercia downloads page](https://opercia.ai/download):
...
After installing, sign in with the same email you use on the web. Once signed in, Desktop starts its own bundled Opercia CLI and detects the AI coding tools already installed on the machine.

<Callout type="info">
The CLI bundled with Desktop only serves the app's managed runtime. If you also want to run commands like `opercia issue` in a terminal, install the CLI separately following [step 2 of the quickstart](/cloud-quickstart#2-connect-a-computer).
</Callout>
...
After sign-in, Desktop creates a dedicated CLI profile for the current Opercia service and uses it to start the daemon. The profile lives at:

```text
~/.opercia/profiles/desktop-<host>/
```

It never reads or overwrites the default profile you use in the terminal. If you also start another daemon manually, Opercia shows them as separate runtimes.
...
Desktop connects to Opercia Cloud by default. To connect to a self-hosted instance, create `desktop.json` inside the `.opercia` directory of your home directory:

| Platform | Path |
| --- | --- |
| macOS | `/Users/<you>/.opercia/desktop.json` |
| Linux | `/home/<you>/.opercia/desktop.json` |
| Windows | `C:\Users\<you>\.opercia\desktop.json` |
...
This is **not** the CLI's `~/.opercia/config.json`, and the key names differ: the CLI uses `server_url`, Desktop uses `apiUrl`. Desktop never reads the CLI's config — it manages a separate daemon profile under `~/.opercia/profiles/desktop-<host>/`. Editing `config.json` does not change which server Desktop connects to.
</Callout>
...
- **Notepad appends `.txt`.** Saving as `desktop.json` can produce `desktop.json.txt`, which Desktop treats as not found. Turn on file name extensions in Explorer, or pick *All files* in the save dialog. To see the real filename, run `Get-ChildItem "$env:USERPROFILE\.opercia" -Filter "desktop.json*"` in PowerShell — the `Name` column must read exactly `desktop.json`.
...
## Windows Defender flags Opercia as a virus
...
```text
C:\Users\<you>\AppData\Local\Programs\@operciadesktop\resources\app.asar.unpacked\resources\bin\opercia.exe
```

**This is a false positive.** The `!ml` suffix means the verdict came from Defender's machine-learning heuristics rather than a malware signature. Opercia's Windows builds are not Authenticode code-signed yet, and a freshly published unsigned binary that starts background processes and opens network connections is exactly the profile those heuristics score as suspicious. Every Windows artifact is built by GitHub Actions from the public source in this repository.

**Verify it yourself** — each GitHub release publishes a `checksums.txt` covering every CLI archive:

```powershell
Get-FileHash .\opercia-cli-<version>-windows-amd64.zip -Algorithm SHA256
```

Compare the hash with the matching line in `checksums.txt` on the [latest release](https://github.com/opercia-ai/opercia/releases/latest).

**How to fix**:

1. **Restore the quarantined file** — Windows Security → *Virus & threat protection* → *Protection history* → select the Opercia item → *Actions* → *Restore*.
2. **Add an exclusion** so it isn't quarantined again — *Virus & threat protection* → *Manage settings* → *Exclusions* → *Add an exclusion* → *Folder*, and add both:
   - `%LOCALAPPDATA%\Programs\@operciadesktop`
   - `%APPDATA%\Opercia`

   Both paths matter: when the bundled CLI goes missing, Desktop downloads a replacement into `%APPDATA%\Opercia\bin`, so excluding only the install folder leaves you in a loop where the replacement gets quarantined too.
...
<Callout type="warning">
Only add these exclusions if you installed Opercia from the [downloads page](https://opercia.ai/download) or [GitHub releases](https://github.com/opercia-ai/opercia/releases) and the checksum matches. An exclusion disables real-time protection for everything in that folder.
</Callout>
