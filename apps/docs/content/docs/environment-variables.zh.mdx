---
title: 环境变量
description: 自托管 Operica 常用的服务端、存储、集成和运行时配置。
---

import { Callout } from "fumadocs-ui/components/callout";

Operica 在进程启动时读取环境变量。修改后通常需要重启对应的 API、Web 或守护进程。Docker Compose 的 `docker compose restart` 不会重新读取 `.env`，要用 `up -d` 重建容器才能生效。
...
不要在生产环境使用默认的 `JWT_SECRET`，也不要设置 `OPERICA_DEV_VERIFICATION_CODE`。
</Callout>
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `DATABASE_URL` | 本地 `operica` 数据库 | PostgreSQL 连接地址 |
...
| `LOG_LEVEL` | 应用默认 | 日志级别 |
| `OPERICA_SHUTDOWN_HOLD_DURATION` | `0` | 收到终止信号后，开始优雅退出前等待多久 |
...
| `OPERICA_APP_URL` | 回退到 `FRONTEND_ORIGIN` | 用户可访问的 Web 地址；CLI 登录和账号绑定链接使用它 |
| `OPERICA_PUBLIC_URL` | 空 | 公网 API 地址；用于 webhook URL 和运行时连接说明 |
...
| `RESEND_FROM_EMAIL` | `noreply@operica.ai` | 发件地址，必须属于已验证域名 |
...
| `OPERICA_DEV_VERIFICATION_CODE` | 空 | 非 production 环境使用的固定 6 位测试验证码 |
| `AUTO_LOGIN_EMAIL` | 空 | 非 production 环境的桌面端自动登录账号；留空即可关闭 |

白名单的准确判断顺序见[登录与注册](/auth-setup#注册范围控制)。

在受控的本地开发环境中设置 `AUTO_LOGIN_EMAIL=operica@operica.local` 后，服务端会在需要时创建该用户，并让无凭证请求使用该账号，桌面端也会自动获取 JWT。`APP_ENV=production` 时该配置会被忽略，禁止在公网部署启用。

## 附件存储

没有设置 `S3_BUCKET` 时，Operica 使用本地磁盘。
...
| `OPERICA_TRUSTED_PROXIES` | 空 | 自动化 webhook 与实时连接使用的可信代理 CIDR |
...
| 飞书 | `OPERICA_LARK_SECRET_KEY` | base64 编码的 32 字节凭据加密密钥 |
| Slack | `OPERICA_SLACK_SECRET_KEY` | base64 编码的 32 字节 token 加密密钥 |
| Composio | `COMPOSIO_API_KEY` | 启用 Composio 工具连接 |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | 回调 API 地址；可回退到 `OPERICA_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state 签名密钥；可从 `JWT_SECRET` 派生 |
| 自托管 Git | `OPERICA_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab 集成开关，compose 默认开启 |
| 自托管 Git | `OPERICA_VCS_SECRET_KEY` | base64 编码的 32 字节加密密钥（`openssl rand -base64 32`）；未配置时该功能整体不可用 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `OPERICA_LLM_API_KEY` | 空 | OpenAI 兼容 API key |
| `OPERICA_LLM_BASE_URL` | 空 | OpenAI 兼容 endpoint |
| `OPERICA_LLM_DEFAULT_MODEL` | `gpt-5.6-luna` | 请求未指定模型时使用 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `OPERICA_SERVER_URL` | `ws://localhost:8080/ws` | Operica API / WebSocket 地址，也接受 `http(s)` |
| `OPERICA_DAEMON_DEVICE_NAME` | 主机名 | 运行时列表中的设备名 |
| `OPERICA_AGENT_RUNTIME_NAME` | `Local Agent` | 运行时显示名 |
| `OPERICA_DAEMON_POLL_INTERVAL` | `30s` | 没有唤醒事件时的执行任务轮询间隔 |
| `OPERICA_DAEMON_HEARTBEAT_INTERVAL` | `15s` | 心跳间隔 |
| `OPERICA_DAEMON_MAX_CONCURRENT_TASKS` | `20` | 单个守护进程的并发执行任务上限 |
| `OPERICA_AGENT_TIMEOUT` | `0` | 单次执行的绝对时限；`0` 表示不设置 |
| `OPERICA_AGENT_IDLE_WATCHDOG` | `30m` | 没有输出且没有工具执行时的静默上限 |
| `OPERICA_AGENT_TOOL_WATCHDOG` | `2h` | 单个工具调用持续静默的上限 |
| `OPERICA_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode 专用静默阈值 |
| `OPERICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex 语义静默阈值 |
| `OPERICA_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server 启动握手上限 |
| `OPERICA_DAEMON_AUTO_UPDATE` | Cloud `true`；自托管 `false` | 是否自动检查并更新 CLI |
| `OPERICA_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | 检查更新的间隔 |
| `OPERICA_DAEMON_AUTO_RELOAD` | `true` | 是否在 `operica` 二进制被带外替换（`brew upgrade`、重新下载、本地构建）后重启到新二进制。与 `OPERICA_DAEMON_AUTO_UPDATE` 相互独立 |
| `OPERICA_WORKSPACES_ROOT` | `~/operica_workspaces` | 执行任务工作目录的根目录 |
| `OPERICA_KEEP_ENV_AFTER_TASK` | `false` | 保留执行任务目录用于调试 |

各 AI 编程工具可以使用 `OPERICA_<PROVIDER>_PATH` 和 `OPERICA_<PROVIDER>_MODEL` 覆盖命令路径与默认模型。QwenPaw 是例外：它没有 `OPERICA_QWENPAW_MODEL`，因为 Operica 不会向它传模型，详见 [AI 编程工具对比](/providers#模型来源)。全机默认参数 `OPERICA_<PROVIDER>_ARGS` 目前支持 Claude Code、Codex、CodeBuddy、Qwen Code 和 QwenPaw 五款工具。对应变量是 `OPERICA_CLAUDE_ARGS`、`OPERICA_CODEX_ARGS`、`OPERICA_CODEBUDDY_ARGS`、`OPERICA_QWEN_ARGS` 和 `OPERICA_QWENPAW_ARGS`。例如：
...
优先级为命令行 flag → 环境变量 → `~/.operica/config.json` → 内置默认。watchdog 的行为见[守护进程与运行时](/daemon-runtimes)。

### 守护进程配置的持久化

守护进程侧的常用配置也可以写入 `~/.operica/config.json`，不必依赖 shell 环境变量；命名 profile 的配置文件在 `~/.operica/profiles/<name>/config.json`：
...
| 键 | 默认值 | 说明 |
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Operica API / WebSocket 地址 |
...
- 传空字符串清除已持久化的值，回到环境变量或内置默认，例如 `operica config set poll_interval ""`。
