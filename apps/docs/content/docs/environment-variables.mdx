...
description: Common server, storage, integration, and runtime configuration for self-hosting Operica.
---

import { Callout } from "fumadocs-ui/components/callout";

Operica reads environment variables at process startup. After changing one, restart the affected API, web, or daemon process. Docker Compose's `docker compose restart` does not re-read `.env`; recreate containers with `up -d` for changes to take effect.
...
```dotenv
DATABASE_URL=postgres://user:password@postgres:5432/operica?sslmode=require
JWT_SECRET=<long-random-secret>
APP_ENV=production
FRONTEND_ORIGIN=https://operica.example.com
OPERICA_APP_URL=https://operica.example.com
OPERICA_PUBLIC_URL=https://api.operica.example.com
```
...
Do not use the default `JWT_SECRET` in production, and do not set `OPERICA_DEV_VERIFICATION_CODE`.
</Callout>
...
|---|---|---|
| `DATABASE_URL` | local `operica` database | PostgreSQL connection string |
...
| `OPERICA_SHUTDOWN_HOLD_DURATION` | `0` | How long to wait after a termination signal before graceful shutdown begins |
...
| `OPERICA_APP_URL` | falls back to `FRONTEND_ORIGIN` | User-reachable web URL; CLI sign-in and account-link URLs use it |
| `OPERICA_PUBLIC_URL` | empty | Public API URL; used for webhook URLs and runtime connection instructions |
...
| `RESEND_FROM_EMAIL` | `noreply@operica.ai` | Sender address; must belong to a verified domain |
...
| `OPERICA_DEV_VERIFICATION_CODE` | empty | Fixed 6-digit test code for non-production environments |
| `AUTO_LOGIN_EMAIL` | empty | Desktop auto-login identity for non-production environments; leave empty to disable |

For the exact allowlist evaluation order, see [Sign-in and signup](/auth-setup#signup-restrictions).

Set `AUTO_LOGIN_EMAIL=operica@operica.local` for controlled local development. The server creates the user if needed, attributes unauthenticated requests to it, and lets Desktop obtain a JWT automatically. This setting is ignored when `APP_ENV=production`; never enable it on a public deployment.

## Attachment storage

When `S3_BUCKET` is unset, Operica uses local disk.
...
| `OPERICA_TRUSTED_PROXIES` | empty | Trusted proxy CIDRs for automation webhooks and realtime connections |
...
| Lark | `OPERICA_LARK_SECRET_KEY` | Base64-encoded 32-byte credential encryption key |
| Slack | `OPERICA_SLACK_SECRET_KEY` | Base64-encoded 32-byte token encryption key |
| Composio | `COMPOSIO_API_KEY` | Enables Composio tool connections |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | Callback API URL; can fall back to `OPERICA_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state signing secret; can be derived from `JWT_SECRET` |
| Self-hosted Git | `OPERICA_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab integration switch; enabled by default in compose |
| Self-hosted Git | `OPERICA_VCS_SECRET_KEY` | Base64-encoded 32-byte encryption key (`openssl rand -base64 32`); the feature is entirely unavailable without it |
...
| Variable | Default | Description |
|---|---|---|
| `OPERICA_LLM_API_KEY` | empty | OpenAI-compatible API key |
| `OPERICA_LLM_BASE_URL` | empty | OpenAI-compatible endpoint |
| `OPERICA_LLM_DEFAULT_MODEL` | `gpt-5.6-luna` | Used when a request does not specify a model |
...
| Variable | Default | Description |
|---|---|---|
| `OPERICA_SERVER_URL` | `ws://localhost:8080/ws` | Operica API / WebSocket URL; also accepts `http(s)` |
| `OPERICA_DAEMON_DEVICE_NAME` | hostname | Device name shown in the runtime list |
| `OPERICA_AGENT_RUNTIME_NAME` | `Local Agent` | Runtime display name |
| `OPERICA_DAEMON_POLL_INTERVAL` | `30s` | Task polling interval when no wake event arrives |
| `OPERICA_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `OPERICA_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Concurrent task ceiling per daemon |
| `OPERICA_AGENT_TIMEOUT` | `0` | Absolute time limit per run; `0` means no limit |
| `OPERICA_AGENT_IDLE_WATCHDOG` | `30m` | Silence ceiling with no output and no tool execution |
| `OPERICA_AGENT_TOOL_WATCHDOG` | `2h` | Silence ceiling for a single tool call |
| `OPERICA_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode-specific silence threshold |
| `OPERICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex semantic-silence threshold |
| `OPERICA_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server startup handshake ceiling |
| `OPERICA_DAEMON_AUTO_UPDATE` | Cloud `true`; self-hosted `false` | Whether to check for and apply CLI updates automatically |
| `OPERICA_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | Update check interval |
| `OPERICA_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `operica` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `OPERICA_DAEMON_AUTO_UPDATE` |
| `OPERICA_WORKSPACES_ROOT` | `~/operica_workspaces` | Root directory for task working directories |
| `OPERICA_KEEP_ENV_AFTER_TASK` | `false` | Keep task directories for debugging |

Each AI coding tool accepts `OPERICA_<PROVIDER>_PATH` and `OPERICA_<PROVIDER>_MODEL` to override the command path and default model. QwenPaw is the exception: it has no `OPERICA_QWENPAW_MODEL`, because Operica never sends it a model — see [AI coding tools comparison](/providers#model-sources). Machine-wide default arguments via `OPERICA_<PROVIDER>_ARGS` are currently supported for five tools: Claude Code, Codex, CodeBuddy, Qwen Code, and QwenPaw. The variables are `OPERICA_CLAUDE_ARGS`, `OPERICA_CODEX_ARGS`, `OPERICA_CODEBUDDY_ARGS`, `OPERICA_QWEN_ARGS`, and `OPERICA_QWENPAW_ARGS`. For example:

```dotenv
OPERICA_CLAUDE_PATH=/opt/bin/claude
OPERICA_CLAUDE_ARGS=--max-turns 40
```

Precedence is command-line flag → environment variable → `~/.operica/config.json` → built-in default. For watchdog behavior, see [Daemon and runtimes](/daemon-runtimes).

### Persisting daemon configuration

Common daemon-side settings can also be written to `~/.operica/config.json` instead of relying on shell environment variables; named profiles keep theirs at `~/.operica/profiles/<name>/config.json`:

```bash
operica config set poll_interval 10s
operica config show
```
...
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Operica API / WebSocket URL |
...
- Passing an empty string clears a persisted value and falls back to the env var or built-in default, for example `operica config set poll_interval ""`.
