...
description: Common server, storage, integration, and runtime configuration for self-hosting Opercia.
---

import { Callout } from "fumadocs-ui/components/callout";

Opercia reads environment variables at process startup. After changing one, restart the affected API, web, or daemon process. Docker Compose's `docker compose restart` does not re-read `.env`; recreate containers with `up -d` for changes to take effect.
...
```dotenv
DATABASE_URL=postgres://user:password@postgres:5432/opercia?sslmode=require
JWT_SECRET=<long-random-secret>
APP_ENV=production
FRONTEND_ORIGIN=https://opercia.example.com
OPERCIA_APP_URL=https://opercia.example.com
OPERCIA_PUBLIC_URL=https://api.opercia.example.com
```
...
Do not use the default `JWT_SECRET` in production, and do not set `OPERCIA_DEV_VERIFICATION_CODE`.
</Callout>
...
|---|---|---|
| `DATABASE_URL` | local `opercia` database | PostgreSQL connection string |
...
| `OPERCIA_SHUTDOWN_HOLD_DURATION` | `0` | How long to wait after a termination signal before graceful shutdown begins |
...
| `OPERCIA_APP_URL` | falls back to `FRONTEND_ORIGIN` | User-reachable web URL; CLI sign-in and account-link URLs use it |
| `OPERCIA_PUBLIC_URL` | empty | Public API URL; used for webhook URLs and runtime connection instructions |
...
| `RESEND_FROM_EMAIL` | `noreply@opercia.ai` | Sender address; must belong to a verified domain |
...
| `OPERCIA_DEV_VERIFICATION_CODE` | empty | Fixed 6-digit test code for non-production environments |

For the exact allowlist evaluation order, see [Sign-in and signup](/auth-setup#signup-restrictions).

## Attachment storage

When `S3_BUCKET` is unset, Opercia uses local disk.
...
| `OPERCIA_TRUSTED_PROXIES` | empty | Trusted proxy CIDRs for automation webhooks and realtime connections |
...
| Lark | `OPERCIA_LARK_SECRET_KEY` | Base64-encoded 32-byte credential encryption key |
| Slack | `OPERCIA_SLACK_SECRET_KEY` | Base64-encoded 32-byte token encryption key |
| Composio | `COMPOSIO_API_KEY` | Enables Composio tool connections |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | Callback API URL; can fall back to `OPERCIA_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state signing secret; can be derived from `JWT_SECRET` |
| Self-hosted Git | `OPERCIA_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab integration switch; enabled by default in compose |
| Self-hosted Git | `OPERCIA_VCS_SECRET_KEY` | Base64-encoded 32-byte encryption key (`openssl rand -base64 32`); the feature is entirely unavailable without it |
...
| Variable | Default | Description |
|---|---|---|
| `OPERCIA_LLM_API_KEY` | empty | OpenAI-compatible API key |
| `OPERCIA_LLM_BASE_URL` | empty | OpenAI-compatible endpoint |
| `OPERCIA_LLM_DEFAULT_MODEL` | `gpt-5.6-luna` | Used when a request does not specify a model |
...
| Variable | Default | Description |
|---|---|---|
| `OPERCIA_SERVER_URL` | `ws://localhost:8080/ws` | Opercia API / WebSocket URL; also accepts `http(s)` |
| `OPERCIA_DAEMON_DEVICE_NAME` | hostname | Device name shown in the runtime list |
| `OPERCIA_AGENT_RUNTIME_NAME` | `Local Agent` | Runtime display name |
| `OPERCIA_DAEMON_POLL_INTERVAL` | `30s` | Task polling interval when no wake event arrives |
| `OPERCIA_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `OPERCIA_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Concurrent task ceiling per daemon |
| `OPERCIA_AGENT_TIMEOUT` | `0` | Absolute time limit per run; `0` means no limit |
| `OPERCIA_AGENT_IDLE_WATCHDOG` | `30m` | Silence ceiling with no output and no tool execution |
| `OPERCIA_AGENT_TOOL_WATCHDOG` | `2h` | Silence ceiling for a single tool call |
| `OPERCIA_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode-specific silence threshold |
| `OPERCIA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex semantic-silence threshold |
| `OPERCIA_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server startup handshake ceiling |
| `OPERCIA_DAEMON_AUTO_UPDATE` | Cloud `true`; self-hosted `false` | Whether to check for and apply CLI updates automatically |
| `OPERCIA_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | Update check interval |
| `OPERCIA_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `opercia` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `OPERCIA_DAEMON_AUTO_UPDATE` |
| `OPERCIA_WORKSPACES_ROOT` | `~/opercia_workspaces` | Root directory for task working directories |
| `OPERCIA_KEEP_ENV_AFTER_TASK` | `false` | Keep task directories for debugging |

Each AI coding tool accepts `OPERCIA_<PROVIDER>_PATH` and `OPERCIA_<PROVIDER>_MODEL` to override the command path and default model. QwenPaw is the exception: it has no `OPERCIA_QWENPAW_MODEL`, because Opercia never sends it a model — see [AI coding tools comparison](/providers#model-sources). Machine-wide default arguments via `OPERCIA_<PROVIDER>_ARGS` are currently supported for five tools: Claude Code, Codex, CodeBuddy, Qwen Code, and QwenPaw. The variables are `OPERCIA_CLAUDE_ARGS`, `OPERCIA_CODEX_ARGS`, `OPERCIA_CODEBUDDY_ARGS`, `OPERCIA_QWEN_ARGS`, and `OPERCIA_QWENPAW_ARGS`. For example:

```dotenv
OPERCIA_CLAUDE_PATH=/opt/bin/claude
OPERCIA_CLAUDE_ARGS=--max-turns 40
```

Precedence is command-line flag → environment variable → `~/.opercia/config.json` → built-in default. For watchdog behavior, see [Daemon and runtimes](/daemon-runtimes).

### Persisting daemon configuration

Common daemon-side settings can also be written to `~/.opercia/config.json` instead of relying on shell environment variables; named profiles keep theirs at `~/.opercia/profiles/<name>/config.json`:

```bash
opercia config set poll_interval 10s
opercia config show
```
...
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Opercia API / WebSocket URL |
...
- Passing an empty string clears a persisted value and falls back to the env var or built-in default, for example `opercia config set poll_interval ""`.
