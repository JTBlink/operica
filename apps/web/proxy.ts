...
import { LOCALE_COOKIE } from "@operica/core/i18n";
import {
...
// Forward the resolved locale to RSC layouts via the `x-operica-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(OPERICA_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("operica_logged_in");
  ...
  // https://operica.ai/ must remain a public-site navigation even when a local
