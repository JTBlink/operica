...
import { LOCALE_COOKIE } from "@opercia/core/i18n";
import {
...
// Forward the resolved locale to RSC layouts via the `x-opercia-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(OPERCIA_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("opercia_logged_in");
  ...
  // https://opercia.ai/ must remain a public-site navigation even when a local
