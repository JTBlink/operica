...
import { StatusIcon, PriorityIcon } from "@opercia/views/issues/components";
import { STATUS_CONFIG } from "@opercia/core/issues/config/status";
import { PRIORITY_CONFIG } from "@opercia/core/issues/config/priority";
import type { IssueStatus, IssuePriority } from "@opercia/core/types";
    ...
    <div className="relative aspect-video overflow-hidden rounded-lg border bg-background text-foreground shadow-2xl">
      ...
      <div className="flex h-10 shrink-0 items-center border-b bg-background px-4 text-body">
        <div className="flex items-center gap-1.5 min-w-0 text-caption">
          <span className="text-muted-foreground">Opercia Demo</span>
...
const mockToolCalls = [
  ...
  { type: "tool_result" as const, preview: "ok  \tgithub.com/opercia/server/internal/handler\t0.847s" },
];
