 ...
 *   operica-desktop-{version}-mac-{arch}.{dmg|zip}
 *   operica-desktop-{version}-windows-{arch}.exe
 *   operica-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^operica-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
