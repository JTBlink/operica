...
// (keyed by the app name "Operica Canary"). The env hooks to override both
...
// "/tmp/operica-143" is 211362000, and 211362000 % 1000 === 0). Range 5174–6173.
...
// The dev app then shows e.g. "Operica Canary mul-3724-194" in Cmd+Tab and gets
...
// a basename at different paths (e.g. /a/operica vs /b/operica) or whose names
...
// the primary checkout keeps the unchanged 5173 / "Operica Canary" defaults.
  ...
  if (log) {
    console.log(
      `[dev:desktop] worktree isolation → renderer port ${env.DESKTOP_RENDERER_PORT}, ` +
        `app "Operica Canary ${env.DESKTOP_APP_SUFFIX}"`,
    );
  }
  return env;
}
