...
import { CoreProvider } from "@operica/core/platform";
import { pickLocale, type SupportedLocale } from "@operica/core/i18n";
import { useAuthStore } from "@operica/core/auth";
import { useWelcomeStore } from "@operica/core/onboarding";
import { workspaceKeys, workspaceListOptions } from "@operica/core/workspace/queries";
import { api } from "@operica/core/api";
import { useHasOnboarded } from "@operica/core/paths";
import { setCurrentWorkspace } from "@operica/core/platform";
import { ThemeProvider } from "@operica/ui/components/common/theme-provider";
...
import { captureEvent } from "@operica/core/analytics";
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.operica config).
  useEffect(() => {
    if (!runtimeConfig) return;
    window.daemonAPI.setTargetApiUrl(runtimeConfig.apiUrl);
  }, [runtimeConfig]);

  // Listen for invite IDs delivered via deep link (operica://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (operica://auth/callback?token=...).
    ...
    <div className="flex h-screen items-center justify-center bg-background p-8 text-foreground">
      <div className="max-w-xl rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-body text-muted-foreground">
          Operica Desktop could not load <code>~/.operica/desktop.json</code>. Fix or remove the file and restart the app.
        </p>
