...
// operica CLI can find agent binaries like claude/codex/opencode. Must run
...
function installLocaleRefresh(window: BrowserWindow): void {
  // Electron has no dedicated OS-language event. Check whenever any Operica
...
// single-worktree case still lands at "Operica Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Operica ${process.env.DESKTOP_APP_SUFFIX}`
  : "Operica";
...
if (is.dev) {
  ...
  // to "Operica", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.operica.desktop.dev" : "ai.operica.desktop",
    );
