...
const PREFS_PATH = join(homedir(), ".opercia", "desktop_prefs.json");
...
// `opercia daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/opercia/cmd_daemon.go) to
 ...
 * a stuck start is an auth problem. Hits the same endpoint `opercia auth status`
 ...
 * the user's default CLI profile at `~/.opercia/` — neither its files nor its
 * health port.
 */
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["opercia.exe"] : ["opercia"];
 ...
 *   to `apps/desktop/resources/bin/opercia`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Opercia.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "opercia.exe" : "opercia";
...
/**
 * Returns a usable `opercia` binary path. Priority:
 ...
 *   5. `opercia` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Opercia Desktop" }),
  });
  if (!res.ok) {
...
async function savePrefs(prefs: DaemonPrefs): Promise<void> {
  const dir = join(homedir(), ".opercia");
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, OPERCIA_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "opercia CLI is not installed" };
