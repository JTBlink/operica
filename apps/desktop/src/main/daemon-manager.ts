...
const PREFS_PATH = join(homedir(), ".operica", "desktop_prefs.json");
...
// `operica daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/operica/cmd_daemon.go) to
 ...
 * a stuck start is an auth problem. Hits the same endpoint `operica auth status`
 ...
 * the user's default CLI profile at `~/.operica/` — neither its files nor its
 * health port.
 */
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["operica.exe"] : ["operica"];
 ...
 *   to `apps/desktop/resources/bin/operica`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Operica.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "operica.exe" : "operica";
...
/**
 * Returns a usable `operica` binary path. Priority:
 ...
 *   5. `operica` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Operica Desktop" }),
  });
  if (!res.ok) {
...
async function savePrefs(prefs: DaemonPrefs): Promise<void> {
  const dir = join(homedir(), ".operica");
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, OPERICA_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "operica CLI is not installed" };
