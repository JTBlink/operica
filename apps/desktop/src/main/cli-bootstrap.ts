...
// Desktop prefers the bundled `opercia` CLI shipped inside the app for
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `opercia-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `opercia` binary. If one is already present at
