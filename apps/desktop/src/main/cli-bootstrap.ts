...
// Desktop prefers the bundled `operica` CLI shipped inside the app for
...
function binaryName(): string {
  return process.platform === "win32" ? "operica.exe" : "operica";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `operica-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `operica` binary. If one is already present at
