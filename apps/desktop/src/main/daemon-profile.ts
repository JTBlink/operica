...
// Keep the Go impl in sync: server/cmd/opercia/cmd_daemon.go healthPortForProfile.
export const DEFAULT_HEALTH_PORT = 19514;

/**
 * Desktop owns only `~/.opercia/profiles/desktop-<host>/`. The default profile
 * at `~/.opercia/` — config, daemon log, and health port 19514 — belongs to the
...
export function profileDir(profile: string): string {
  assertResolvedProfile(profile);
  return join(homedir(), ".opercia", "profiles", profile);
}
...
// Sidecar file that records which Opercia user the cached PAT in config.json
 ...
 * user's default profile at `~/.opercia/config.json`.
 */
