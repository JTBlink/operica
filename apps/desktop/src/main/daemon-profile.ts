...
// Keep the Go impl in sync: server/cmd/operica/cmd_daemon.go healthPortForProfile.
export const DEFAULT_HEALTH_PORT = 19514;

/**
 * Desktop owns only `~/.operica/profiles/desktop-<host>/`. The default profile
 * at `~/.operica/` — config, daemon log, and health port 19514 — belongs to the
...
export function profileDir(profile: string): string {
  assertResolvedProfile(profile);
  return join(homedir(), ".operica", "profiles", profile);
}
...
// Sidecar file that records which Operica user the cached PAT in config.json
 ...
 * user's default profile at `~/.operica/config.json`.
 */
